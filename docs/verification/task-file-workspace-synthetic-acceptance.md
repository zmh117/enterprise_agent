...
日期：2026-08-15。证据级别：仓库内 synthetic acceptance；不包含真实业务文件、真实 Secret、真实钉钉调用或目标环境部署声明。

## 新鲜链路

`backend/tests/test_task_file_workspace_synthetic_acceptance.py` 使用合成 UTF-8 TXT、内存对象存储和假钉钉 Delivery，贯通：
...
| UTF-8、BOM、15 MiB、20 文件、100 MiB | `test_file_workspace_core.py`, `test_file_commit_streaming.py` |
