...
Web 只治理 Python Agent；数据库和服务继续使用多 Agent 的 definition、draft revision、immutable publication 模型。新 Job 在创建时固定 publication ID、`python-v1` runtime kind、revision 和 hash，后续发布或回滚不会改变已经创建的 Job。
