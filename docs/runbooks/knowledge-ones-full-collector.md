...
本入口结束于 `STAGED` 候选。后续清洗分块、Embedding、Qdrant 完整性校验、受管自动切版和默认停用的每小时 worker 属于后续阶段；没有完成这些阶段前，不能把候选计数当作当前可检索内容，也不要启用定时采集。真实 ONES 的过滤、排序、项目范围及版本戳合同需在目标环境单次运行验收后，才可授权每小时调度。
