...
本页描述 `enable-governed-knowledge-retrieval` 已实现的来源、资源 Web 管理、角色应用 KB 允许范围、Job/ONES 内部投影、平台双身份桥与目录/检索应用服务，以及正式工具合同、固定 Runtime 路由、独立 MCP 服务入口与可选部署配置，不代表 Agent 检索已经开放。120 秒截止与阻塞 I/O 取消已完成代码及隔离故障测试，尚未部署；完整隔离服务组合和真实验收仍须完成。仅部署 Embedding/Qdrant 不会自动发布资源或赋予业务权限。

## 数据库和部署边界

2026-09-20 新增内容连接拆分：平台保留逻辑 KB、授权/发布、Job 和审计；内容 PostgreSQL 的 `knowledge` schema 与 Qdrant 可在 Web 配置。migration 140 增加仅含连接配置/凭据引用的字段并解除平台本地索引外键；旧发布原 hash 和数据保持不变。独立内容库不要求平台身份/Job/迁移账本，也不会自动建立或搬迁。部署及账号要求见[内容存储配置](../../knowledge/README.md#web-配置独立内容存储)。本机已部署平台侧与 Web 改造；在线 Knowledge MCP 和真实外部内容库接入仍未部署或验收。用户随后确认内容账号可以具备管理/写权限，现有外部读取连接仍强制只读事务；该账号策略修改须更新配套代码后生效，不代表新增内容编辑能力。
...
API/Worker/ONES 镜像更新仍须经过正式 Migrator 和平台 schema readiness 门禁。2026-09-20 经用户批准，本机已从 139 升级至 140、更新相关服务和 Web、恢复 Embedding/Qdrant；5000 文档、8309 分块/向量点及已有知识资源 revision=6 的发布记录保持不变。本轮未保存草稿、发布或新增授权，也未部署在线 Knowledge MCP。仅此本机部署证据不能替代真实 Agent 检索验收。不要在未升级 schema 时只替换后端，也不要在新库上直接回滚到只认识旧 schema 的镜像。停用知识检索时保留表、文档、索引和向量卷，不执行降级删表或重编码。
...
- **任务 7.3 已完成隔离验收，未部署**：首次鉴权、连接池/锁、SQL/fetch/事务完成与网络均受预算控制。PostgreSQL 轮询取消并淘汰异常连接，正常连接恢复原超时设置；DNS 仅把域名解析交给可终止、无业务内容的短生命周期子进程，IP 不建子进程，不改 Host/TLS。ONES 正文回放保留真实断开语义。120 秒是业务结果截止，并非所有资源必须同一瞬间消失；驱动取消、子进程/临时池回收及失败审计各允许至多 1 秒有界收尾，不再读取或返回业务结果。迟到线程实际退出前仍占用槽位。真实环境网络/ONES/Agent 延迟仍需 10.4/11 验收，不以这些故障测试冒充上线。
