...
平台 Secret、短期附件凭据和受管渠道凭据使用同一个固定 Master Key 加密。
...
紧急情况下只允许停机后的离线事务重加密，详见
[Master Key 紧急离线重加密](./emergency-master-key-reencryption.md)。该流程
不是周期轮换功能，平台不会提供 Web 管理、多 Key keyring 或到期策略。
