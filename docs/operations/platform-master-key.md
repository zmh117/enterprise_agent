...
仓库当前没有覆盖全部密文域的受支持重加密工具。现有密文域至少包括
`platform_secret_version`、ONES `external_identity_credential` 和 pending challenge、
`message_attachment` 下载凭据以及 `channel_ingress_event` reply credential。发生泄漏或
文件损坏时，只能停机、隔离、备份并执行事件响应；在另行实现并验证覆盖全部密文域的
原子工具前，不得执行数据库重加密或替换 Key 文件，否则仍存续密文将无法解密。
平台不提供 Web 管理、多 Key keyring 或到期策略。
