...
- 逐block置信度只在Docling明确提供时保存；未提供时v2以`null`和“上游未提供”展示，不伪造数值也不把整图判失败。
- 不启用 VLM、远程图片描述、自定义模型、插件、任意 URL 或运行时模型下载。

## 上线前门禁

1. 确认工作区、目标环境和变更窗口，保留未提交改动；不得输出 Secret、JWT、对象键或业务正文。
2. 确认数据库 migration head 为 `116_expand_office_embedded_image_layout_ocr.sql`，且目标构建包含 forward-only `117_expand_docling_layout_ocr_v2.sql`。
3. 验证 `docling-layout-ocr-v2` 的代码 hash、Compose 配置 hash 和模型 artifact digest 精确一致；模型目录缺失、空目录或 digest 不一致必须停止。
...
2. 先运行 migrator应用 migration 117；禁止删除 migration ledger、逆向 DDL 或把失败迁移标记为成功。
...
1. 新建只用于验收的 Application Revision/Publication，冻结 `docling-layout-ocr-v2` 的精确 code/version/hash；不得改写历史 Publication。
...
1. 停止创建或激活新的 `docling-layout-ocr-v2` Publication；将后续业务切换到一个已验证的 `docling-text-v1` 或 `NONE` Publication。不得原地修改历史快照/hash。
2. 排空或安全终结 layout parent、picture item、assembly、staging、outbox 与 cleanup 队列；同一任务不得同时由新旧 Worker 消费。
3. 回退到仍兼容 migration 117 schema 的已验证应用镜像。保留 migration 116/117、历史 run、Representation、asset/occurrence/item 身份和审计；禁止 down-migrate、删除 ledger 行或把 layout run 解释为 text run。
