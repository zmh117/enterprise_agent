...
from app.modules.knowledge.ones_export import ExportValidationError
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        result = VectorService(database, embedding, qdrant).query(args.index_code, args.knowledge_base_code,
                                                                 query, top_k=args.top_k)
        print(json.dumps({'event': 'knowledge_query_completed', **result}, ensure_ascii=False), flush=True)
        return 0
        ...
        print('{"event":"knowledge_query_failed","error_code":"knowledge_schema_head_mismatch"}', flush=True)
        return 1
    except Exception:
        print('{"event":"knowledge_query_failed","error_code":"knowledge_vector_unavailable"}', flush=True)
        return 1
    finally:
