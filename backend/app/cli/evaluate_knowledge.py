...
from app.modules.knowledge.evaluation import OfflineEvaluation
from app.modules.knowledge.evaluation_dataset import EvaluationError, load_dataset
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        parser = SafeArgumentParser(description='本地离线知识评测；仅输出指标/摘要，不是业务授权验收')
        parser.add_argument('--dataset', required=True, type=Path)
        parser.add_argument('--index-code')
        parser.add_argument('--validate-only', action='store_true', help='仅校验标注集，不连接数据库或模型')
        args = parser.parse_args(argv)
        if bool(args.index_code) == args.validate_only:
            raise EvaluationError('knowledge_evaluation_arguments_invalid')
        dataset = load_dataset(args.dataset)
        if args.validate_only:
            print(json.dumps({'event': 'knowledge_evaluation_dataset_valid', 'dataset_hash': dataset.digest,
                              'basis': dataset.basis, 'cases': len(dataset.cases),
                              'business_acceptance': False}), flush=True)
            return 0
        ...
        report = OfflineEvaluation(VectorService(database, embedding, qdrant)).run(dataset, args.index_code)
        print(json.dumps({'event': 'knowledge_evaluation_completed', **report}, ensure_ascii=False), flush=True)
