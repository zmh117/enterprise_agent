...
from app.modules.knowledge.ones_export import ExportValidationError, identifier
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        service = VectorService(database, embedding, qdrant)
        snapshot = service.repository.snapshot(service.repository.scope(args.knowledge_base_code),
                    args.chunk_profile_hash, args.expected_documents, args.expected_chunks)
        ...
        if args.commit:
            result = service.build(args.index_code, snapshot,
                progress=lambda counts: print(json.dumps({'event': 'vector_progress', **counts}), flush=True))
