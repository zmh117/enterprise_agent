...
def load_master_key_file(path: str, *, required: bool) -> str:
    ...
    if not configured:
        if required:
            raise MasterKeyConfigurationError(
                "APP_CONFIG_MASTER_KEY_FILE is required"
            )
        return ""
    ...
    try:
        ...
        raise MasterKeyConfigurationError(
            "Configured Master Key file was not found"
        ) from exc
    ...
    if _is_container_secret(key_path):
        if mode & 0o222:
            raise MasterKeyConfigurationError(
                "Container Master Key file must be read-only"
            )
    elif mode & 0o077:
        raise MasterKeyConfigurationError(
            "Master Key file permissions must be owner-only"
        )
    ...
    try:
        ...
        raise MasterKeyConfigurationError(
            "Master Key file is unreadable"
        ) from exc
    if content.endswith("\n"):
        content = content[:-1]
    if "\n" in content or "\r" in content:
        raise MasterKeyConfigurationError(
            "Master Key file must contain exactly one line"
        )
    if not content.startswith(MASTER_KEY_PREFIX):
        raise MasterKeyConfigurationError(
            "Master Key file format version is invalid"
        )
    ...
    try:
        ...
        raise MasterKeyConfigurationError(
            "Master Key file payload is invalid"
        ) from exc
    if len(decoded) != MASTER_KEY_BYTES:
        raise MasterKeyConfigurationError(
            "Master Key file must contain 32 bytes"
        )
    canonical = base64.urlsafe_b64encode(decoded).decode("ascii").rstrip("=")
    if encoded != canonical:
        raise MasterKeyConfigurationError(
            "Master Key file payload is not canonical base64url"
        )
    return canonical
