...
_add(
    _read_contract(
        ...
        (
            ...
            "用户可访问的有界结果。名称关键词必须来自用户；用户未提供有意义的名称关键词"
            "却要求列出全部时必须先追问，不得自行使用“表”或“表格”等宽泛词猜测。查询时"
            "使用 page_size=50；若 truncated=true 且返回 next_cursor，必须保持同一 query"
            "继续分页，按完整 base_id 去重，累计最多 4 页或 200 条；仅在达到该上限后仍有"
            "下一页，或 Provider 未返回可用 next_cursor 时，才向用户说明结果仍被截断。"
        ),
        _object(
            {
