    ...
    if file_job:
        ...
        restrictions.extend(
            [
                ...
                "File MCP and file_manifest timestamps (source_received_at, version_created_at, observed_at, representation_created_at, expires_at) are canonical UTC RFC 3339 machine values. When speaking to the user, convert instants to the user's display timezone when needed; never reinterpret a Z or +00:00 wall-clock as local time. For relative upload-time requests, compare source_received_at with the service-provided observed_at; do not treat version_created_at or a generic created_at as upload time.",
