...
def profile() -> dict[str, Any]:
    return {
        "contract": "knowledge-dense/v1", "model_id": MODEL["model_id"],
        "revision": MODEL["revision"], "artifact_hash": fingerprint(MODEL),
        "runtime_hash": hashlib.sha256((PACKAGE / "embedding_runtime.lock").read_bytes()).hexdigest(),
        "dimension": DIMENSION, "dtype": "float32", "normalize": True,
        "pooling": "official-sentence-transformers", "distance": "Cosine", "query_prefix": "",
        "max_items": MAX_ITEMS, "max_tokens": MAX_TOKENS, "max_batch_tokens": MAX_BATCH_TOKENS,
    }


def texts_from_request(value: Any) -> list[str]:
    if not isinstance(value, dict) or set(value) != {"texts", "profile_hash"}:
        raise VectorError("knowledge_embedding_input_invalid")
    if value["profile_hash"] != fingerprint(profile()):
