...
from app.modules.knowledge.evaluation_dataset import EvaluationDataset, EvaluationError
from app.modules.knowledge.storage import table
from app.modules.knowledge.vector_contract import VectorError, fingerprint
...
def score_documents(relevant: frozenset[str], retrieved: list[str], *,
                    visible_relevant: frozenset[str] | None = None) -> QueryMetrics:
...
def latency_summary(values: list[float]) -> dict[str, float | None]:
    ...
    return {key: round(ordered[max(0, math.ceil(len(ordered) * p) - 1)], 3) if ordered else None
            for key, p in (('p50_ms', .5), ('p95_ms', .95))}


def _failure(exc: Exception) -> str:
    # Only code-owned categories are emitted, never arbitrary exception messages/codes.
    if isinstance(exc, VectorError):
        if exc.code in {'knowledge_vector_source_invalid', 'knowledge_vector_source_changed',
                        'knowledge_vector_point_conflict', 'knowledge_collection_conflict'}:
            return 'source_or_index_changed'
        if exc.code in {'knowledge_embedding_profile_mismatch', 'knowledge_vector_index_not_ready'}:
            return 'index_unavailable'
        if exc.code in {'knowledge_vector_unavailable', 'knowledge_embedding_busy',
...
def implementation_hash() -> str:
    ...
    code_files = ('evaluation.py', 'evaluation_dataset.py', 'vector_service.py', 'vector_repository.py',
                  'vector_clients.py', 'vector_contract.py')
    return fingerprint({name: hashlib.sha256((package / name).read_bytes()).hexdigest() for name in code_files})
...
class OfflineEvaluation:
    ...
    def _catalog(self, dataset: EvaluationDataset, index: dict[str, Any]) -> dict[str, str]:
        """Only current documents represented by INDEXED chunks in this exact corpus are labels."""
        def t(name: str) -> str:
            return table(self.database, name)
        source = self.database.execute_one(f'select id,source_system from {t("source")} where id=?',
                                            (dataset.source_id,))
    ...
    def run(self, dataset: EvaluationDataset, index_code: str) -> dict[str, Any]:
        ...
        if not index or index['state'] != 'READY' or index['knowledge_base_id'] != repo.scope(dataset.base_code):
            raise EvaluationError('knowledge_evaluation_index_invalid')
        repo.assert_current(index)
        catalog = self._catalog(dataset, index)
        ...
        for case, relevant in zip(dataset.cases, labels, strict=True):
            ...
            try:
                ...
                if any(doc not in catalog for doc in retrieved) or result['index_id'] != index['id']:
        ...
        repo.assert_current(index)
        if repo.get(index_code) != index or self._catalog(dataset, index) != catalog:
            raise EvaluationError('knowledge_evaluation_corpus_changed')
        if implementation_hash() != code_hash:
            raise EvaluationError('knowledge_evaluation_code_changed')
        total = len(dataset.cases)
        return {
            'contract': 'knowledge-evaluation-report/v1', 'mode': 'offline_dense',
            'basis': dataset.basis, 'business_acceptance': False, 'authorization_verified': False,
            'recall_scope': 'complete_labels' if dataset.labels_complete else 'labeled_set_only',
            'dataset_hash': dataset.digest, 'dataset_version_hash': fingerprint(dataset.version),
            'code_hash': code_hash, 'started_at': started,
            'environment': {'python': platform.python_version(), 'os': platform.system(), 'arch': platform.machine()},
            'index_id': index['id'], 'knowledge_base_id': index['knowledge_base_id'], 'source_id': dataset.source_id,
            ...
            'cases': total, 'categories': dict(sorted(categories.items())), 'answerable_cases': len(recalls),
            'recall_at_10': sum(recalls) / len(recalls) if recalls else None,
            'mrr_at_10': sum(reciprocal) / len(reciprocal) if reciprocal else None,
            'no_answer': {'total': sum(case.no_answer for case in dataset.cases),
                          'completed': no_answer_completed, 'false_positives': no_answer_false_positive,
                          'false_positive_rate': no_answer_false_positive / no_answer_completed
                          if no_answer_completed else None},
            'retrieval_latency': latency_summary(durations), 'end_to_end_latency': None,
            'partial_rate': partial / total, 'bounded_rate': bounded / total,
            'unmatched_answerable_cases': unmatched,
            'failures': dict(sorted(failures.items())), 'failed_cases': sum(failures.values()),
        }
