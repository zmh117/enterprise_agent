...
def test_repository_migration_catalog_has_unique_ordered_versions_and_checksums() -> None:
    ...
    assert [(item.version, item.name) for item in catalog] == [
        ...
        ("129", "129_expand_external_action_proposal_chains.sql"),
        ("130", "130_restore_dingtalk_identity_indexes.sql"),
        ...
        ("136", "136_expand_sandbox_v2_capacity.sql"),
        ("137", "137_expand_knowledge_retrieval_governance.sql"),
        ("138", "138_expand_knowledge_source_confirmation.sql"),
        ("139", "139_relax_knowledge_resource_source_binding.sql"),
        ("140", "140_expand_knowledge_storage_connections.sql"),
    ]
...
def test_schema_cursor_migration_upgrades_134_without_rewriting_jobs(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert result.applied == ("135", "136", "137", "138", "139", "140")
...
def test_agent_run_audit_expand_keeps_job_lifecycle_separate() -> None:
    ...
    assert str(protocol["dflt_value"]).strip("'") == "1.5"
    audit_columns = {row["name"] for row in database.execute("pragma table_info(agent_run_audit)")}
    assert {
        "invocation_id",
        "audit_sha256",
    ...
    audit_fks = database.execute("pragma foreign_key_list(agent_run_audit)")
...
def test_external_action_intent_separates_confirmation_route_from_execution_provider() -> None:
    ...
    assert "uq_external_action_intent_mcp_call" in indexes
    assert "uq_external_action_intent_supersedes" in indexes
...
def test_confirmation_card_template_migration_backfills_connector_and_outbox(
    ...
    foreign_key_violations_before = {
        (str(row["table"]), int(row["rowid"]), str(row["parent"]))
    ...
    assert result.applied == ("128", "129", "130", "131", "132", "133", "134", "135", "136", "137", "138", "139", "140")
    ...
    preserved_intent = database.execute_one(
        """
        select status, arguments_json, proposal_chain_id, supersedes_intent_id,
               provider_attempt_status
          from external_action_intent where id = ?
        """,
        ("action-card-history",),
    )
    assert preserved_intent == {
        "status": "PENDING_CONFIRMATION",
        "arguments_json": "{}",
        "proposal_chain_id": "action-card-history",
    ...
    repeated = Migrator(
        database,
        default_migrations_dir(),
        migrator_build="card-binding-repeat",
    ).run()
...
def test_explicit_fresh_contract_remains_supported_after_release() -> None:
    ...
    assert SchemaHeadValidator(database, default_migrations_dir()).require_current() == "140"
    assert (
        Migrator(
            database,
            default_migrations_dir(),
...
def test_release_unbound_ones_identity_migration_preserves_history_and_constraints(
    ...
    assert result.applied == ("126", "127", "128", "129", "130", "131", "132", "133", "134", "135", "136", "137", "138", "139", "140")
    assert database.execute_one(
        """
...
def test_final_schema_comment_manifest_covers_every_owned_table_and_column() -> None:
    ...
    table_pattern = re.compile(
        r"COMMENT\s+ON\s+TABLE\s+(?:public\.)?\"?((?:knowledge\.)?[a-z0-9_]+)\"?\s+IS\s+"
        r"'((?:''|[^'])*)'\s*;",
        re.IGNORECASE | re.DOTALL,
    )
    column_pattern = re.compile(
        r"COMMENT\s+ON\s+COLUMN\s+(?:public\.)?\"?((?:knowledge\.)?[a-z0-9_]+)\"?\."
        r"\"?([a-z0-9_]+)\"?\s+IS\s+'((?:''|[^'])*)'\s*;",
        re.IGNORECASE | re.DOTALL,
    )
    ...
    assert len({table for table in owned_tables if table.startswith("knowledge.")}) == 15
    database.close()
...
def test_schema_head_validator_is_read_only_and_rejects_missing_ledger() -> None:
    ...
    with pytest.raises(
        SchemaHeadError,
        match="ledger is missing; expected head 140",
    ):
        SchemaHeadValidator(
            database,
            default_migrations_dir(),
