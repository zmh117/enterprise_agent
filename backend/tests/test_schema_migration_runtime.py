...
def test_explicit_fresh_contract_remains_supported_after_release() -> None:
    ...
    assert SchemaHeadValidator(database, default_migrations_dir()).require_current() == "117"
    assert (
        Migrator(
            database,
            default_migrations_dir(),
...
def test_final_schema_comment_manifest_covers_every_owned_table_and_column() -> None:
    ...
    assert len(owned_tables) == 121
    assert len(owned_columns) == 1575
    database.close()
...
def test_schema_head_validator_is_read_only_and_rejects_missing_ledger() -> None:
    ...
    with pytest.raises(
        SchemaHeadError,
        match="ledger is missing; expected head 117",
    ):
        SchemaHeadValidator(
            database,
            default_migrations_dir(),
