...
def test_confirmation_card_template_migration_backfills_connector_and_outbox(
    ...
    assert result.applied == ("128", "129", "130", "131")
...
def test_explicit_fresh_contract_remains_supported_after_release() -> None:
    ...
    assert SchemaHeadValidator(database, default_migrations_dir()).require_current() == "131"
    assert (
        Migrator(
            database,
            default_migrations_dir(),
...
def test_release_unbound_ones_identity_migration_preserves_history_and_constraints(
    ...
    assert result.applied == ("126", "127", "128", "129", "130", "131")
    assert database.execute_one(
        """
...
def test_final_schema_comment_manifest_covers_every_owned_table_and_column() -> None:
    ...
    assert len({column for column in owned_columns if not column[0].startswith("knowledge.")}) == 1734
    assert len({table for table in owned_tables if table.startswith("knowledge.")}) == 7
    database.close()
...
def test_schema_head_validator_is_read_only_and_rejects_missing_ledger() -> None:
    ...
    with pytest.raises(
        SchemaHeadError,
        match="ledger is missing; expected head 131",
    ):
        SchemaHeadValidator(
            database,
            default_migrations_dir(),
