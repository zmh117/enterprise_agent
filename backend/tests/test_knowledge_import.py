...
from app.modules.knowledge.import_service import KnowledgeImportService, stable_id, table
from app.modules.knowledge.ones_export import ExportValidationError, Sanitizer, prepare_export
...
def export_row(index: int = 1, *, target: str | None = None) -> tuple[dict, dict]:
    ...
    listing = {
        "uuid": key, "name": f"合成缺陷 {index}", "number": index, "createTime": STAMP,
        "project": {"uuid": "project_id"}, "status": {"uuid": "status_id", "name": "处理中"},
        "sprint": {"uuid": "sprint_id", "name": "合成迭代"},
    }
    detail = {
        "uuid": key, "summary": listing["name"], "number": index,
        "create_time": STAMP, "server_update_stamp": STAMP + 100,
        "project_uuid": "合成项目", "status_uuid": "处理中", "sprint_uuid": "合成迭代",
        "issue_type_uuid": "缺陷", "desc": "操作步骤：输入 a < b\n实际结果：合成错误\n预期结果：合成成功",
        ...
        "discussion_count": 2, "attachment_count": 1, "related_tasks": [], "links": [],
    }
    if target:
        detail["links"] = [{"task_uuid": target, "task_link_type_uuid": "synthetic_link_type", "link_desc_type": "link_out_desc"}]
        detail["related_tasks"] = [{"uuid": target, "summary": "不得导入的关联摘要", "readable": True}]
...
def run(database, prepared, **kwargs):
    return KnowledgeImportService(database).import_export(
        prepared, source_code="synthetic_source", knowledge_base_code="synthetic_base", **kwargs,
    )
...
def test_mapping_plain_text_and_no_attachment_processing(tmp_path):
    ...
    assert record.values["attributes"]["image_references"][0]["source_attachment_id"] == "synthetic_image"


def test_secret_cleaning():
    sanitizer = Sanitizer()
    value = sanitizer.value({
        "access_token": "synthetic-do-not-persist", "note": "正常正文\nPASSWORD=synthetic-secret\n后续文字",
        "url": "postgresql://synthetic:synthetic@localhost/test", "data": "data:image/gif;base64,synthetic",
...
def test_duplicate_json_keys_and_oversize_rows_rejected(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("app.modules.knowledge.ones_export.MAX_ROW_BYTES", 16)
...
def test_import_idempotence_and_dangling_targets(database, tmp_path):
    prepared = prepare(tmp_path, [export_row(target="synthetic_item_2"), export_row(2, target="missing_requirement")])
    first = run(database, prepared)
    second = run(database, prepared)
    assert first["created_count"] == 2
    assert second["replayed"] is True
    assert first["verification"] == second["verification"] == {
        "source_documents": 2, "defect_documents": 2, "matching_input_documents": 2,
        "source_revisions": 2, "memberships": 2, "relation_observations": 2,
        "resolved_relation_targets": 1, "unresolved_relation_targets": 1,
    }
...
def test_document_can_be_in_two_bases_without_copying(database, tmp_path):
    ...
    result = KnowledgeImportService(database).import_export(
        prepared, source_code="synthetic_source", knowledge_base_code="another_base",
    )
...
def test_same_source_stamp_conflict_is_rejected(database, tmp_path):
    ...
    rows = database.execute('select state,processed_count,error_code from "knowledge.import_run" where state=\'failed\'')
    assert rows == [{"state": "failed", "processed_count": 0, "error_code": "knowledge_source_version_conflict"}]
...
def test_interrupted_run_resumes_committed_checkpoint(database, tmp_path, monkeypatch):
    ...
    original = KnowledgeImportService._record
    calls = 0
    ...
    monkeypatch.setattr(KnowledgeImportService, "_record", interrupted)
    with pytest.raises(ExportValidationError, match="^knowledge_import_failed$"):
        run(database, prepared)
    assert database.execute_one('select processed_count from "knowledge.import_run"')["processed_count"] == 1
    monkeypatch.setattr(KnowledgeImportService, "_record", original)
...
def test_current_pointer_cannot_reference_another_document(database, tmp_path):
    ...
    with pytest.raises(Exception):
        with database.unit_of_work():
            database.execute('update "knowledge.document" set current_revision_id=? where id=?', (docs[1]["current_revision_id"], docs[0]["id"]))
...
def test_cli_preflight_does_not_load_database_settings(tmp_path, monkeypatch, capsys):
    ...
    assert main(["--input-dir", str(tmp_path), "--detail-file", "detail.jsonl", "--list-file", "list.jsonl", "--expected-count", "1"]) == 0
    output = capsys.readouterr().out
    assert "合成缺陷" not in output
    assert "preflight_passed" in output


@pytest.mark.skipif(not os.getenv("KNOWLEDGE_TEST_POSTGRES_DSN"), reason="requires isolated PostgreSQL")
def test_real_postgres_migration_import_constraints_and_lock(tmp_path):
    ...
    try:
        ...
        assert sum(key.startswith("knowledge.") for key in comments["tables"]) == 7
        ...
        service = KnowledgeImportService(database)
        with service._source_lock(source_id):
            with pytest.raises(ExportValidationError, match="knowledge_source_import_busy"):
                with KnowledgeImportService(contender)._source_lock(source_id):
                    pytest.fail("second connection must not acquire the source lock")
        first = prepare(tmp_path, [export_row(target="synthetic_item_2")])
        imported = service.import_export(first, source_code=source_code, knowledge_base_code=base_code)
        assert imported["verification"]["unresolved_relation_targets"] == 1
        assert service.import_export(first, source_code=source_code, knowledge_base_code=base_code)["replayed"]
        later = prepare(tmp_path, [export_row(2)])
        imported = service.import_export(later, source_code=source_code, knowledge_base_code=base_code)
        assert imported["verification"]["source_revisions"] == 2
        assert imported["verification"]["resolved_relation_targets"] == 1
        docs = database.execute("select id,current_revision_id from knowledge.document where source_id=? order by id", (source_id,))
        row = database.execute_one("select source_snapshot,source_created_at from knowledge.document_revision where id=?", (docs[0]["current_revision_id"],))
        ...
        with pytest.raises(Exception):
            with database.unit_of_work():
                database.execute("update knowledge.document_relation set to_external_id='wrong_synthetic_target' where from_source_id=?", (source_id,))
        with pytest.raises(Exception):
            with database.unit_of_work():
                database.execute("update knowledge.document_revision set attributes='invalid-json' where id=?", (docs[0]["current_revision_id"],))
        assert service.import_export(later, source_code=source_code, knowledge_base_code=base_code)["verification"]["source_revisions"] == 2
    finally:
        contender.close()
        database.close()
