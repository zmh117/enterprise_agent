...
from app.modules.knowledge.chunking import DEFAULT_PROFILE, clean_text, prepare_chunks, validate_chunks
from app.modules.knowledge.import_service import KnowledgeImportService
from app.modules.knowledge.ones_export import ExportValidationError, NORMALIZER_VERSION, digest
from app.modules.knowledge.storage import table
...
def record(body="步骤：提交合成订单。\n实际：E_TEST。\n预期：成功。", solution="增加合成参数校验。"):
    return {"normalizer_version": NORMALIZER_VERSION, "title": "合成缺陷", "source_project_name": "合成项目",
            "body_text": body, "attributes": {"solution_text": solution, "module_names": ["合成模块"]},
...
def test_short_problem_whole_solution_separate_and_citation_coordinates():
    ...
    for chunk in result.chunks:
        assert chunk["evidence_text"] == result.normalized_fields[chunk["source_field"]][chunk["source_start"]:chunk["source_end"]]
...
def test_long_content_never_loses_nonwhitespace_or_tail(body):
    ...
    assert all(covered[i] or char.isspace() for i, char in enumerate(result.normalized_fields["body_text"]))
...
@pytest.mark.parametrize("solution,state", [(None,"absent"),(" \n", "absent"),("已修复。", "status_only"),
                                            ("已处理", "status_only"),({}, "unsupported_type")])
...
def test_duplicate_solution_and_short_but_meaningful_solution():
    assert prepare_chunks(record("合成问题", "合成问题")).quality["solution_state"] == "duplicates_body"
...
def test_random_boundary_coverage_and_determinism():
    ...
    for _ in range(70):
        text = "".join(rng.choice(["合成", "E_DEMO", "\n", "\n\n", "    x < y", "。 ", "```", "\t"]) for _ in range(rng.randrange(1, 1800)))
        if text.strip():
...
def import_rows(database, tmp_path, rows, *, source="synthetic_source", base="synthetic_base"):
    return KnowledgeImportService(database).import_export(prepare(tmp_path, rows), source_code=source, knowledge_base_code=base)


def run(database, *, count=1, commit=True, profile=DEFAULT_PROFILE, base="synthetic_base", **kwargs):
    return ChunkService(database, profile=profile).run(knowledge_base_code=base, expected_count=count, commit=commit, **kwargs)


def source_fingerprint(database):
    data = {}
    for name in ("source","document","document_revision","knowledge_base","knowledge_base_document","import_run","document_relation"):
...
def test_revision_and_profile_version_create_new_sets(database, tmp_path):
    ...
    assert run(database, profile=replace(DEFAULT_PROFILE, version="ones-text-chunks/test-v2"))["counts"]["created"] == 1
    assert len(database.execute('select id from "knowledge.document_chunk_set"')) == 3


def test_middle_chunk_write_failure_rolls_back_whole_set_then_resumes(database, tmp_path, monkeypatch):
    import_rows(database, tmp_path, [export_row(), export_row(2)])
    import app.modules.knowledge.chunk_service as module
...
@pytest.mark.parametrize("target,column", [("document_chunk", "evidence_hash"), ("document_chunk_set", "profile_hash")])
...
def test_real_postgres_chunk_storage_constraints_and_replay(tmp_path):
    ...
    try:
        ...
        sets=db.execute('select id,document_id,document_revision_id,normalized_fields from knowledge.document_chunk_set where document_id in (select id from knowledge.document where source_id=(select id from knowledge.source where code=?)) order by id',(source,))
        assert isinstance(sets[0]["normalized_fields"],dict)
        with pytest.raises(Exception):
            with db.unit_of_work():
                db.execute('update knowledge.document_chunk_set set document_id=? where id=?',(sets[1]["document_id"],sets[0]["id"]))
        with pytest.raises(Exception):
            with db.unit_of_work():
