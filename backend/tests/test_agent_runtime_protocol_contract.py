...
CONTRACT_ROOT = Path(__file__).parents[2] / "contracts" / "agent-runtime" / "v1.3"
...
def test_only_current_runtime_contract_is_published_and_hash_matches() -> None:
    assert SUPPORTED_RUNTIME_PROTOCOL_VERSIONS == ("1.3",)
...
def test_current_empty_file_context_request_is_valid() -> None:
    request = _request()
    assert request["protocol_version"] == "1.3"
...
def test_current_safe_runtime_fixture_covers_runtime_contracts() -> None:
    ...
    for name, key in keys.items():
        ...
        validate_runtime_contract(name, fixture[key], protocol_version="1.3")
