...
class RabbitMQ4IntegrationTests(unittest.TestCase):
    ...
    def tearDown(self) -> None:
        if not hasattr(self, "channel"):
            return
        for queue_name in (self.queues.job_queue, self.queues.dead_queue):
            self.channel.queue_delete(queue=queue_name)
        self.connection.close()

    def test_publisher_is_compatible_with_rabbitmq4_outbox_target_queue(self) -> None:
        ...
        publisher.publish_agent_job("event-smoke", "job-smoke", "corr-job")
