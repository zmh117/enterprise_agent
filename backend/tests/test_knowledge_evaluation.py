...
from app.modules.knowledge.evaluation import OfflineEvaluation, latency_summary, score_documents
from app.modules.knowledge.evaluation_dataset import (
    MAX_DATASET_BYTES, EvaluationError, load_dataset, parse_dataset,
)
from app.modules.knowledge.vector_contract import VectorError
...
def sample():
    return {
        'contract': 'knowledge-evaluation/v1', 'dataset_version': 'synthetic-v1',
        'basis': 'synthetic', 'labels_complete': False, 'knowledge_base_code': 'synthetic_base',
        'source_id': '00000000-0000-0000-0000-000000000001',
        'annotation': {'origin': 'synthetic_fixture', 'reviewed': True},
        'cases': [{'query_id': 'q1', 'query': '合成问题不得进入汇总', 'category': 'paraphrase',
                   'no_answer': False, 'relevant': [{'kind': 'work_item', 'id': 'synthetic_item_1'}]}],
    }
...
def test_document_metrics_dedupe_top10_misses_and_visible_denominator():
    ...
    assert scores.recall == 1 and scores.reciprocal_rank == .5
    assert score_documents(relevant, ['a']).recall == .5
...
def test_offline_runner_reuses_kernel_without_writes_and_redacts(prepared, capsys, caplog):
    ...
    value['cases'].append({'query_id': 'q2', 'query': '合成无答案', 'category': 'no_answer',
                           'no_answer': True, 'relevant': []})
    ...
    assert report['parameters'] == {'top_k': 10, 'candidate_limit': 200, 'metrics_version': 'document-v1'}
...
def test_failures_are_counted_not_silent_misses_or_no_answer_success(prepared, monkeypatch):
    ...
    assert report['no_answer']['completed'] == 0 and report['no_answer']['false_positive_rate'] is None
...
def test_cli_validates_without_database_and_redacts_all_errors(tmp_path, monkeypatch, capsys):
    ...
    assert cli.main(['--dataset', str(tmp_path / 'hidden-labels'), '--index-code', 'synthetic']) == 1
...
def test_real_label_directory_is_git_ignored():
    ...
    result = subprocess.run(['git', 'check-ignore', '.local/knowledge-evaluation/labels.json'],
