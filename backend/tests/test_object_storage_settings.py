...
def test_application_settings_only_expose_governed_file_storage(
    ...
    monkeypatch.setenv("FILE_STORAGE_LEGACY_ATTACHMENT_BUCKET", "legacy-attachments")
    monkeypatch.setenv(
        "FILE_STORAGE_ACCESS_KEY_REF", "secret://platform/file-storage-access"
    )
    monkeypatch.setenv(
        "FILE_STORAGE_SECRET_KEY_REF", "secret://platform/file-storage-secret"
    )
    ...
    assert settings.file_service.legacy_attachment_bucket == "legacy-attachments"
