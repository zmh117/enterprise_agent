...
def test_sandbox_v2_environment_uses_expanded_defaults_without_version_bump(monkeypatch) -> None:
    ...
    monkeypatch.setenv("PYTHON_AGENT_RUNTIME_SANDBOX_MAX_WORK_OUTPUT_FILES", "16")
    with pytest.raises(ValueError, match="sandbox-v2"):
        _sandbox_limits_from_environment()
...
def test_sandbox_v2_readiness_requires_expanded_available_space(tmp_path, monkeypatch, available, status) -> None:
    ...
    assert response.json()["sandbox_max_files"] == 128
    assert response.json()["sandbox_max_work_output_files"] == 80
...
def test_python_runtime_normalizes_tool_budget_exhaustion_across_sdk_paths(
    ...
    # This test exercises SDK budget interrupts, not MCP transport. Real bridge
    ...
    client = FixedMcpClaudeSdkClient(
        tool_result_bridge_factory=lambda **kwargs: BudgetBridge(),
        limits=build_settings().execution,
        api_key="runtime-only-model-secret",
        mcp_server_url="http://tool-mcp:9103/mcp",
        runtime_build_identity=BuildIdentity(
