...
from app.modules.knowledge.chunking import DEFAULT_PROFILE
from app.modules.knowledge.storage import source_lock, table
from app.modules.knowledge.vector_clients import EmbeddingClient, InternalHttp, QdrantClient
from app.modules.knowledge.vector_contract import VectorError, fingerprint, profile, validate_vector
...
class SyntheticQdrant:
    ...
    def search(self, index, vector, limit):
        ...
        return [dict(p, score=1.0) for p in sorted(self.points.values(), key=lambda p: p['id'])[:limit]]
...
def prepared(tmp_path):
    ...
    chunks = ChunkService(db).run(knowledge_base_code='synthetic_base', expected_count=2, commit=True)['counts']['chunks']
    service = VectorService(db, SyntheticEmbedding(), SyntheticQdrant())
    snapshot = service.repository.snapshot(service.repository.scope('synthetic_base'), DEFAULT_PROFILE.fingerprint, 2, chunks)
...
def test_new_revision_filters_stale_points_and_requires_new_index_version(prepared, tmp_path):
    ...
    ChunkService(db).run(knowledge_base_code='synthetic_base', expected_count=2, commit=True)
    current = service.repository.snapshot(snapshot['knowledge_base_id'], DEFAULT_PROFILE.fingerprint,
                                         2, snapshot['expected_chunk_count'])
    with pytest.raises(VectorError, match='index_conflict'):
...
def test_query_deduplication_expands_until_enough_documents(prepared, monkeypatch):
    ...
    first = list(service.repository.rows(index['knowledge_base_id'], index['chunk_profile_hash']))[0]
    # 仅合成候选；前 80 个都定位同文档，第 81 个命中第二文档。
    second = next(r for r in service.repository.rows(index['knowledge_base_id'], index['chunk_profile_hash'])
    ...
    for i in range(81):
        ...
        mapping[row['id']] = row
        candidates.append({'id': point_id(index, row), 'payload': payload(index, row), 'score': 1-i*.001})
    monkeypatch.setattr(service.repository, 'evidence_many', lambda idx, ids: {cid: mapping[cid] for cid in ids})
    calls = []
...
@pytest.mark.parametrize('vector', [[0.0]*1024, [1.0]*1024, [float('nan')]+[0.0]*1023, [True]+[0.0]*1023, [1.0]])
...
def test_client_profile_order_retry_redirect_and_transaction_boundary(monkeypatch):
    ...
    monkeypatch.setattr('app.modules.knowledge.vector_clients.time.sleep', lambda _: None)
    ...
    http.close()
    client = EmbeddingClient(transport=httpx.MockTransport(lambda r: httpx.Response(200,
        json={'profile_hash': fingerprint(profile()), 'input_hashes': ['wrong'], 'token_counts': [2], 'vectors': [VECTOR]})))
    with pytest.raises(VectorError, match='result_invalid'):
        client.call(['synthetic'], encode=True)
    client.http.close()
    http = InternalHttp('http://knowledge-embedding:8096', transport=httpx.MockTransport(
...
def test_network_io_is_rejected_inside_transaction(prepared):
    db, service, snapshot = prepared
    calls = []
    client = InternalHttp('http://knowledge-embedding:8096', transport=httpx.MockTransport(
...
def test_embedding_client_rejects_malformed_counts_and_vectors(value):
    client = EmbeddingClient(transport=httpx.MockTransport(lambda r: httpx.Response(200,
        json={'profile_hash': fingerprint(profile()), **value})))
    try:
        with pytest.raises(VectorError, match='result_invalid'):
            client.call(['synthetic'], encode=True)
    finally:
...
def test_paged_manifest_covers_more_than_32_rows(tmp_path):
    ...
    try:
        ...
        count = ChunkService(db).run(knowledge_base_code='synthetic_base', expected_count=35, commit=True)['counts']['chunks']
        service = VectorService(db, SyntheticEmbedding(), SyntheticQdrant())
        snapshot = service.repository.snapshot(service.repository.scope('synthetic_base'), DEFAULT_PROFILE.fingerprint, 35, count)
        assert count > 32
...
def test_safe_token_limit_code_is_preserved_without_remote_details():
    client = EmbeddingClient(transport=httpx.MockTransport(lambda r: httpx.Response(422,
        json={'error_code': 'knowledge_embedding_token_limit', 'input': 'hidden synthetic text'})))
    try:
...
def test_qdrant_ownership_conflict_and_unconfirmed_write():
    ...
    client = QdrantClient(transport=httpx.MockTransport(lambda r: httpx.Response(200, json={'result': {'status': 'acknowledged'}})))
...
def test_embedding_api_limits_safe_errors_and_fingerprint():
    with TestClient(create_app(SyntheticEngine())) as client:
        def post(texts, **kw):
            return client.post('/embed', json={'texts': texts, 'profile_hash': fingerprint(profile()), **kw})
        ...
        assert client.post('/tokenize', json={'texts': ['x'*2000]*5, 'profile_hash': fingerprint(profile())}).status_code == 200
...
def test_embedding_api_busy_and_engine_error_do_not_echo():
    ...
    with TestClient(create_app(Blocked())) as client:
        ...
        responses = []
        thread = threading.Thread(target=lambda: responses.append(client.post('/embed', json=request)))
        thread.start()
...
def test_model_file_hash_symlink_missing_and_engine_unavailable(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr('services.knowledge_embedding.engine.EmbeddingEngine', lambda: (_ for _ in ()).throw(RuntimeError('hidden')))
...
def test_compose_runtime_isolation_and_valid_tmpfs():
    config = yaml.load((Path(__file__).resolve().parents[2]/'knowledge/compose.yml').read_text(), Loader=yaml.BaseLoader)
...
@pytest.mark.skipif(not os.getenv('KNOWLEDGE_TEST_POSTGRES_DSN') or not os.getenv('KNOWLEDGE_TEST_QDRANT_URL'),
...
def test_real_postgres_qdrant_replay_constraints_and_repair(tmp_path):
    ...
    try:
        ...
        service = VectorService(db, SyntheticEmbedding(), qdrant)
        snapshot = service.repository.snapshot(service.repository.scope(code), DEFAULT_PROFILE.fingerprint, 2, count)
        competitor = Database(os.environ['KNOWLEDGE_TEST_POSTGRES_DSN'])
        try:
            with source_lock(db, 'vector-index:' + code):
                with pytest.raises(ValueError, match='source_import_busy'):
                    VectorService(competitor, SyntheticEmbedding(), qdrant).build(code, snapshot)
        finally:
            competitor.close()
        ...
        qdrant.http.request('POST', qdrant.path(index)+'/points/delete?wait=true', {'points': [point_id(index, row)]})
        assert service.build(code, snapshot)['encoded'] == 1
        assert len(service.query(code, code, 'synthetic', top_k=2)['documents']) == 2
        with pytest.raises(Exception):
            with db.unit_of_work():
                db.execute(f"update {table(db,'vector_index')} set state='INVALID' where id=?", (index['id'],))
        with pytest.raises(Exception):
            with db.unit_of_work():
                db.execute(f"update {table(db,'vector_index_item')} set chunk_id='missing' where index_id=?", (index['id'],))
    finally:
        qdrant.http.close()
        delegate.close()
        db.close()
