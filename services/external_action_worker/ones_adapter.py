    ...
    def _reauthorize(self, intent: dict[str, Any]) -> tuple[dict[str, Any], Any]:
        ...
        if (
            ...
            raise NonRetryableExecutionError(
                ...
                safe_message="ONES 更新授权或工具合同已变化，请重新发起并确认",
                error_code="ones_task_update_contract_drift",
            )
        if (
            str(intent.get("field_catalog_version") or "") != self.catalog.catalog_version
            or str(intent.get("field_catalog_hash") or "") != self.catalog.content_sha256
        ):
            raise NonRetryableExecutionError(
                "ONES task-update field catalog facts drifted",
                safe_message="缺陷更新字段目录已变化，请重新发起并确认",
                error_code="ones_task_update_catalog_drift",
            )
        ...
        if (
            ...
            raise _authorization_denied(
                "ones_task_update_actor_revoked",
                "当前用户或任务授权已失效，本次更新未执行",
        ...
        if len(matches) != 1:
            raise _authorization_denied(
                "ones_task_update_job_snapshot_revoked",
                "当前 Job 的 ONES 更新授权已失效，本次更新未执行",
        ...
        if confirmation_identity is None or str(
            ...
            raise _authorization_denied(
                "ones_task_update_confirmation_identity_revoked",
                "确认该操作的钉钉身份已失效，本次更新未执行",
        ...
        if identity is None:
            raise _authorization_denied(
                "ones_task_update_identity_revoked",
                "原 ONES 身份已解绑或变更，本次更新未执行",
        ...
        if not isinstance(teams, list) or str(intent["execution_scope_id"]) not in teams:
            raise _authorization_denied(
                "ones_task_update_team_revoked",
                "原确认的 ONES Team 已失效，本次更新未执行",
        ...
        if credentials is None:
            raise _authorization_denied(
                "ones_task_update_credential_unavailable",
                "ONES 身份凭据暂不可用，本次更新未执行",
        ...
        if credential_row is None or str(credential_row.get("status") or "") != "ACTIVE":
            raise _authorization_denied(
                "ones_task_update_reauthentication_required",
                "ONES 身份需要本人重新验证，本次更新未执行",
        ...
        if credential.provider != "ones":
            raise _authorization_denied(
                "ones_task_update_credential_provider_drift",
                "ONES 身份凭据类型已变化，本次更新未执行",
