...
class DocumentPictureRoutes:
    ...
    @denial_on_app_error
    async def picture_asset_content(self, request: Request) -> StreamingResponse:
    ...
    @denial_on_app_error
    async def picture_result_content(self, request: Request) -> StreamingResponse:
        self._access.claims(request, "internal:file-service:document-processing:source:read")
        stream = await asyncio.to_thread(
            self._access.service().open_picture_result,
            picture_item_id=str(request.path_params["picture_item_id"]),
        )
        return StreamingResponse(iter_blocking_stream(stream), media_type="application/json")
