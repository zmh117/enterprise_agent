...
from app.modules.knowledge.vector_contract import (
    MAX_BATCH_TOKENS, MAX_BODY_BYTES, MAX_TOKENS, VectorError, fingerprint,
    profile, texts_from_request,
)
...
def create_app(engine: Any = None) -> FastAPI:
    ...
    async def lifespan(app: FastAPI) -> AsyncIterator[None]:
        if current["engine"] is None:
            try:
                ...
                print('{"event":"embedding_start_failed","error_code":"knowledge_model_unavailable"}', flush=True)
        yield
    ...
    async def ready() -> JSONResponse:
        return JSONResponse({"ready": current["engine"] is not None},
    ...
    async def process(request: Request) -> JSONResponse:
        ...
        try:
            ...
            with anyio.fail_after(15):
                async for part in request.stream():
                    ...
                    if len(body) > MAX_BODY_BYTES:
                        return JSONResponse({"error_code": "knowledge_embedding_body_limit"}, status_code=413)
            try:
                value = json.loads(body)
            except (ValueError, UnicodeError):
                raise VectorError("knowledge_embedding_input_invalid") from None
            texts = texts_from_request(value)
            ...
            result: dict[str, Any] = {"profile_hash": fingerprint(profile()), "token_counts": counts,
