# governed-external-action-confirmation Specification

## Purpose
定义业务 MCP 外部 mutation 的不可变操作意图、原用户逐次确认、异步可恢复执行、卡片 Outbox 和失败关闭边界。

## Requirements

<!-- Integrated from change: `add-governed-dingtalk-mcp-mvp/specs/governed-external-action-confirmation` -->
