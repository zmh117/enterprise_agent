...
- 列表联接 1:1 汇总与已有状态，并只读联接 `app_user` 和 `business_application` 获取受授权页面的当前显示名称；稳定 ID 仍是身份、授权和历史关联事实；
- 页面查询条件收敛为开始时间、结束时间、用户名和应用名；用户名同时匹配 `username/display_name`，应用名同时匹配 `name/code`，名称条件在服务端 SQL 有界查询中执行，不在客户端或取出后伪分页；
