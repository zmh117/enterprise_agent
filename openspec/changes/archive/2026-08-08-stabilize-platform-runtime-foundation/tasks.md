...
- [x] 10.3 展示旧授权数据精确数量/digest 并再次获得用户确认后，事务清理 `permission_policy`、`platform_access_grant` 及其运行时代码/配置
- [x] 10.4 停止旧 Worker/Dispatcher，排空或幂等 backfill pending/retry Job 与 Delivery，隔离无法转换的记录并核验新 Outbox
- [x] 10.5 展示旧 RabbitMQ 拓扑精确清单并再次获得用户确认后，只按精确名称删除已排空且无消费者的旧 exchange/queue/binding
- [x] 10.6 显式导入仍需保留的旧 `env:` Secret 引用并核验新资源引用；不迁移或声明 `vault:`/`kms:` 可用
- [x] 10.7 按 Phase 4 重新生成的 report/prepare 及用户确认执行全量 DB/Redis/Loki reset，并保存 verify 证据
- [x] 10.8 从空配置建立并发布本地验收所需 MySQL 或 SQL Server、Redis、Loki 资源及业务应用 binding；Oracle 保持未发布
- [x] 10.9 完成维护切换核验：无 compatibility、无旧消息消费者、无运行时 YAML 资源 fallback、无悬空 binding、核心服务 ready

## 11. Phase 6：CI、Compose 与真实链路验收

- [x] 11.1 将 CI、Dockerfile 和文档统一到 npm lockfile 与 `npm ci`，移除 pnpm lock 路径并验证冷构建
- [x] 11.2 建立 PR 快速 Gate：后端单元/契约、前端 lint/type/build/test、migration 静态/checksum、OpenSpec 严格校验
- [x] 11.3 建立 Compose 集成 Gate：Migrator 先行、schema head、多个 API/Dispatcher 安全、PostgreSQL/RabbitMQ 中断恢复和健康状态
- [x] 11.4 用无效 Webhook Token 验证不创建 Inbox/Session/Job/Outbox，用缺失 RBAC 验证 fail closed
- [x] 11.5 对 RabbitMQ 中断、Job retry/DEAD、Delivery retry/DEAD、重复消息和精确 CLI replay 执行故障注入并保存证据
- [x] 11.6 扫描 API 响应、数据库普通表、日志、审计、Job、tool-call、Outbox 和前端产物，证明没有 Secret 明文或 Master Key
- [x] 11.7 使用真实本地 Grafana + 标准 Bearer 发送新鲜合成 firing 事件，贯通 Inbox/Job Outbox/RabbitMQ/Worker
- [x] 11.8 使用真实只读 MySQL 或 SQL Server 工具取得受限证据，再贯通结果/Delivery Outbox/真实 DingTalk 回复并保存 correlation 证据
- [x] 11.9 验证受保护 Debug UI 的创建、查询、默认 none 和可选现有 Delivery binding，不依赖公开 API/PAT
- [x] 11.10 在验收报告中明确 HTTP 仅限本地，并将真实 Oracle、Worker RUNNING 崩溃恢复、任务取消、HTTPS/HMAC、Egress、Vault/KMS 标记为未实现/延期
- [x] 11.11 完成 Phase 6 Gate：所有自动化与真实链路证据通过，任何延期能力均未被误报为已完成

## 12. 文档、证据与后续交接

- [x] 12.1 更新本地部署、固定 Master Key、Internal API Token 轮换、资源发布、Outbox/DLQ CLI、维护窗口和故障排查文档
- [x] 12.2 记录六阶段实现证据、数据库与 RabbitMQ 切换摘要、资源 reset verify、真实 E2E correlation 和已知限制
- [x] 12.3 复核 `reset-identity-and-authorization-bootstrap` 仍保持暂停且未被本 change 修改，记录其未来需同步 strict-only 决策
- [x] 12.4 为后续 Worker 执行租约/fencing/取消建立独立 change 候选，不在本次 tasks 中实施
- [x] 12.5 为后续 API Capability Catalog/Handler 管理界面提供稳定的 Handler Registry、Resource Slot 和 Application Publication contract 交接说明
- [x] 12.6 运行最终 OpenSpec 严格校验和全量测试，确认 tasks、specs、design 与实际行为一致后再申请归档
