...
## 第七阶段：测试资产累计10000条（2026-09-10）

- 仅用例库、用例模块、测试计划、用例查询四个列表使用10000条上限，代码共享工具上限映射同时驱动服务收集与输出schema；其他五个列表保持1000。
- bucket单页仍最多200，测试资产分页预算提高到200次，覆盖上游50条短页在200次内读取10000条。其余列表保持50次，直接模块列表仍一次读取后本地限量；所有工具保留90秒、8MiB和既有HTTP/Job/沙盒预算，任何超时或容量失败不伪装完整结果。
- 输入不变，不向模型开放limit/cursor，不人为改变input schema hash；输出数组、returned和cumulative_returned统一使用对应工具上限。必须同步部署ONES与Runtime共享schema，不能混用仍只接受1000条输出的旧Runtime。本轮不改写发布快照；前一阶段旧limit输入契约仍须正常重新发布。
- 回归覆盖四工具（用例查询含模块/计划两种来源）的9999/10000/10001、50条短页与200条页、模块单次列表、其他工具1000条边界、请求/时间/容量保护以及10000条沙盒只读物化和清理。不连接真实ONES、不恢复Mock、不改配置、不提交或推送。

## 第六阶段：固定累计1000条（2026-09-10）

- 九个 GraphQL 列表公开输入移除 limit；非法旧参数按未知字段拒绝，不静默忽略。REST列表不变。
- 自动收集器固定使用 MAX_RESULTS=1000，与 ONES 单页 pagination.limit 分离。服务端逐页请求最多200，直到 hasNextPage=false 或累计1000；短页不能作为终止依据。直接完整列表单次读取并有界截取。既有50次请求、90秒、8MiB和失败关闭保护不变。
- 新 schema hash 只通过重新发布 Agent/Application 进入新 Job；旧快照仍按 drift 拒绝，不自动改写发布记录。本次不连接真实 ONES、不重跑历史 Job、不改环境配置。
- 覆盖全部九工具输入、256/1000/1001总量、上游每页50条仍继续、内部残留limit不能降低总量、短页/失败/预算/游标及旧快照回归。
