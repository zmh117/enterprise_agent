...
- 2026-08-13：修复运行记录详情将后端 1-based `delivery_chunk.chunk_index` 再加一导致单分片显示为 `2/1` 的问题；页面现在直接展示 `chunk_index/chunk_count`，前端合同同步要求分片序号为正整数，并增加 `1/1 · 已送达` 回归。frontend 13 个测试文件、102/102 用例、lint、typecheck 和生产构建均通过；该展示修复不改变投递事实，也不改变 8.1–8.4 状态。
- 2026-08-13：修复 MCP Server 已持久化 `agent_tool_call` 但 Runtime 未回传 tool event 时“实际工具调用”错记为 0 的问题；Worker 现在按当前 invocation 的持久化工具记录与 Runtime 稳定 tool-call identity 取更完整计数，同一调用的 STARTED/SUCCEEDED 生命周期不重复计数。管理查询对当前 invocation 做只读校准，因此旧 Job 无需迁移或回填即可展示正确值。页面将 `2 / 1` 明确标注为“模型轮次 / Runtime 调用：2 次 / 1 次”，并仅从结构化工具结果中展示总量、截断与不受信任标识，不展示 `items` 或 `[object Object]`。后端受影响回归 41/41、前端 103/103、lint、typecheck、生产构建、Ruff（排除既有 F541）和 OpenSpec strict 校验通过；`api-server`、`agent-worker`、`admin-web` 已重建重启，未运行 migration，8.1–8.4 保持未勾选。
- 2026-08-13：运行记录列表现在只读联接系统人员与业务应用，显示“显示名称（@用户名）”和“应用名称（应用编码）”；历史关联缺失时仅使用稳定标识作回退，不改变授权事实。页面筛选收敛为本地开始/结束时间、用户名和应用名，默认过去 24 小时，名称条件在服务端 SQL 中对 `username/display_name` 与 `name/code` 做转义后的模糊匹配，仍先受原有授权范围限制。逐轮模型耗时在 SDK 没有请求起点时明确显示“不可用（SDK 未提供请求起点）”，不用 Job 或 query 总耗时伪造逐轮数值。后端受影响回归 35/35、前端 104/104、lint、typecheck、生产构建、Ruff、PostgreSQL 现场名称匹配和 OpenSpec strict 校验通过；`api-server` 与 `admin-web` 已重建重启且 API healthy。本修复不增加 migration，8.1–8.4 状态不变。
