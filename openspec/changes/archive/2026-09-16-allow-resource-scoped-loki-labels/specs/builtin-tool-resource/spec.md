...
### Requirement: Loki 单次行数配置与资源输入上限
平台 SHALL 将 Loki 单次调用行数默认上限设为 10000；Web 新建资源默认 `max_lines=1000`，管理 API 和 Web 对新建/编辑配置 MUST 统一限制为 1–1000。每次工具调用仍以平台与已发布资源的较小上限校验，省略 `limit` 时使用 100，MUST NOT 将该限制解释为 Job 累计行数预算或自动分页。

#### Scenario: 资源上限生效
- **WHEN** 平台上限 10000、资源上限 1000，Agent 请求 limit=1000
- **THEN** 工具允许单次最多 1000 条，limit=1001 在 Provider 请求前被拒绝

#### Scenario: 配置边界前后端一致
- **WHEN** 新建或编辑资源提交 max_lines=1001
- **THEN** Web 阻止提交，管理 API 拒绝字段值；1000 为合法边界

#### Scenario: 多次查询不是累计行数预算
- **WHEN** 同一 Job 多次请求且每次 limit 均不超过有效上限
- **THEN** 不因累计行数超过 10000 而拒绝；原有工具调用次数、超时和响应容量限制仍生效

#### Scenario: 既有配置不会被默认值覆盖
- **WHEN** 升级前已存在显式 DB/env 平台限制或已发布资源行数配置
- **THEN** 升级不改写这些值，运行仍使用显式平台限制与已发布资源限制的较小值；历史资源重新编辑/验证时适用 1–1000 新输入范围
