...
- 管理端 Job 详情在通过 `jobs.read` 和现有业务范围检查后只返回 invocation 摘要；完整正文由详情页按字段、按有界游标分页延迟读取，Debug evidence、Tool Call、MCP 审计继续保持安全摘要。
...
- 优化 Agent 运行详情：先显示上下文、模型请求、峰值/累计 Token、缓存、成本，以及注册/实际加载/实际调用/自动批准工具数等调优摘要；上下文、模型请求/响应、工具原文和 usage/元数据默认折叠，只有展开的字段才读取当前有界分段，避免超长审计阻塞页面主线程。
...
- Web：扩展 operations schema/API/详情页面与长内容回归测试；完整审计正文不再进入初始 Job 详情响应或未展开 DOM。
- 容量与安全：数据库和按需审计字段响应包含企业正文，初始详情响应不包含；授权仍为 `jobs.read` 加业务范围，运行凭据、认证 Header、Cookie 和 private thinking 不进入审计；上下文来源不保存可派生的重复渲染副本。
