...
- `agent-job-context-audit`: 定义完整 invocation 审计的采集、分块传输、持久化、授权查询与折叠展示。

### Modified Capabilities

- `execution-delivery`: 将完整 invocation 审计纳入成功、失败、超时、重试和 Runtime 恢复链路，同时保持执行、工具、MCP 与 Delivery 事实边界。
...
- 容量与安全：详情响应和数据库将包含企业正文；授权仍为 `jobs.read` 加业务范围，运行凭据不主动进入审计。
