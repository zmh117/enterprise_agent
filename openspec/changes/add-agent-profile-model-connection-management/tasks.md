...
- [ ] 6.4 轮换用户已暴露的现有 Key，确认旧 Secret version不再解析，仓库和运行输出均无该明文
...
- [ ] 6.8 在Web配置并测试轮换后的DeepSeek Anthropic-compatible连接，保存默认Agent草稿、校验并发布新Agent Publication
- [ ] 6.9 验证默认业务应用仍使用旧Agent Publication且钉钉Job行为不变，再显式选择新Publication、发布并激活local Deployment
- [ ] 6.10 创建真实钉钉Job，核对Agent/Business Application Publication、模型连接revision/hash、模型、Provider Host、工具执行和原会话Delivery
- [ ] 6.11 再轮换一次测试Key但不重发Publication，验证后续Job使用新active version且不泄漏Key
