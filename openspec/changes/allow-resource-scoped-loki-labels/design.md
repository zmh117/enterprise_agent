...
6. 行数补充：平台 `MAX_LOKI_LINES` 默认 10000，同步保留入口 `LOKI_MAX_LINES` 的默认值与运行配置目录；资源连接默认 1000，新建/编辑的管理 API 和 Web 均接受 1–1000。实际调用沿用 `min(平台限制, 已发布资源限制)`，limit 默认 100，超出拒绝而非自动夹紧，不累计 Job 行数。资源字段旁说明该语义。
7. 兼容已部署配置：不写数据库迁移、不覆盖 DB/env 显式平台配置，也不改写已有 Published Revision（包括历史高于 1000 的值）；其执行仍受平台与资源较小上限约束。历史资源再次编辑/验证时须改为不超过 1000。仅本次默认值/表单调整不改变 MCP Schema 或 Runtime 协议。
