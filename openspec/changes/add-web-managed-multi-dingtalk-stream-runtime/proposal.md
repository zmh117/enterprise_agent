...
- 后端完成并验证后，再增加“业务应用 → 渠道与触发器”前端页面。第一版只展示和配置两类入口：受管 Webhook、钉钉应用机器人；只有已启用且可作为入口的 Channel 才能在应用 Trigger Binding 中选择。
...
- `business-application-channel-trigger-management`: “业务应用 → 渠道与触发器”页面以及已启用 Channel 到 Trigger Binding 的受控选择行为。
...
- 前端：后端验收完成后增加业务应用下的“渠道与触发器”页面和 Trigger Channel 选择器，不增加其他管理模块。
