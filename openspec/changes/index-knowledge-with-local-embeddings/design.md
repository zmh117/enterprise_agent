...
用户确认修复合并 migration 135 后全量回归发现的四处问题。精确 migration 清单继续由 `test_schema_migration_runtime.py` 集中检查；业务应用与知识向量行为测试按当前 deployable catalog 验证实际 head/应用列表及幂等性，不复制固定版本清单。服务身份直接从共享 token contract 导入大小上限，值和运行规则不变。架构测试保留全生产目录的旧鉴权禁令，仅将 Oracle 专用验证票据类内的 HS256 与 Business Principal 的 Ed25519 分开检查，并补错误落点的反向测试；不更换运行算法、不放宽 Oracle 票据 TTL/用途/防重放门禁。本次仅修复回归与导入边界，不接管分页变更的真实新 Job 验收，不执行正式迁移或全量索引。
