...
- [ ] 7.6 在 Web 输入新的 DeepSeek Key，完成 discover、模型映射、test-draft 和 configure，确认连接由 rotation-required 变为 ready
- [ ] 7.7 核对数据库只新增受管 Secret 引用、加密版本和一个 ready 连接 revision，日志、审计和浏览器可见状态不含 Key
- [ ] 7.8 在 Agent 草稿中选择新连接 revision并完成保存与校验，确认本变更未自动发布 Agent 或切换 Business Application
- [ ] 7.9 验证历史 Agent Publication、已固定 Job 和旧连接 revision保持不可变，并记录 api-server/admin-web 成对回滚步骤
