...
- [x] 10.1 九个 GraphQL 列表移除 MCP 入参 limit，固定累计最多1000，bucket内部每页最多200，直接列表一次获取后限量，保留REST与安全预算
- [x] 10.2 更新调用方与回归，覆盖全部九工具、256/1000/1001、上游50条短页和旧契约拒绝
- [x] 10.3 完成测试、静态、规范及本地部署验证，记录重新发布要求，不连接真实ONES或改写发布快照（421项扩展回归通过，四服务契约一致，容器内50条短页验证通过；见evidence-fixed-collection-limit.md）

## 11. 测试资产累计10000条

- [x] 11.1 四个测试资产列表统一10000条收集/输出上限，最多200次bucket请求，保留其他工具上限和所有时间/容量/安全保护
- [x] 11.2 覆盖四工具及两种用例来源的9999/10000/10001、短页、直接列表、预算失败、非测试工具不变及沙盒物化清理
- [x] 11.3 完成测试、静态、规范及四服务本地部署/容器验证，记录输入哈希不变和真实ONES未验收边界（463项回归通过，四服务契约一致，容器内10000条收集/只读物化/清理通过；见evidence-test-assets-10000.md）
