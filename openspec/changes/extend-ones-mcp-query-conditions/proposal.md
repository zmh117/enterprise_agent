...
- 影响 `backend/app/shared/ones_tool_contracts.py` 与平台 MCP Tool Manifest。
- 影响 `ones_mock/` 的合成响应及 ONES MCP 契约、架构、Mock 和 Runtime 测试。
- 不新增数据库表、Secret 类型、任意 GraphQL/REST 执行器、Agent Skill 或写入 ONES 的能力；既有已发布 Agent/Application/Job 仍需通过新的显式 Publication 才能获得新增 Tool。
