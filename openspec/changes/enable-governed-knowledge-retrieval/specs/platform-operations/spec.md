...
### Requirement: 离线知识来源必须显式核验后才能受治理回源
系统 SHALL 用独立、有修订和审计的来源绑定记录，将离线 source 显式绑定到受信 ONES 实例与 Team。允许检索前 MUST 取得批次来源的明确确认和技术交叉验证，并保留核验状态、操作者、时间与非敏感证据摘要。抽样工作项存在、项目同名或索引完成 MUST NOT 单独证明全批次来源。来源撤销或换版 MUST 使依赖它的检索验证失效并重新发布。
