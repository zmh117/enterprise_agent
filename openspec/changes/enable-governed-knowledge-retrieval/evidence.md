...
| 层级 | 状态 | 证据边界 |
| --- | --- | --- |
| 代码与静态合同 | 已完成当前工程任务 | 按逐轮回归记录，不等于业务效果达标 |
| 合成单元/接口 | 已通过 | 不使用真实业务文本/凭据；详见下方逐轮记录 |
| 隔离数据库/容器 | 已完成任务 10.4 | 正式服务 target 加合成 Provider/语料，不替代真实 Provider |
| 本机在线服务部署 | 已完成部署子项 | 独立账号/服务身份、健康及失败关闭检查；业务配置和新 Job 尚未验收 |
| 真实 ONES 本人权限 | 未完成 | 配置阶段来源确认已取消；仍须两位真实用户逐项权限验收 |
...
## 本机在线 Knowledge MCP 部署（2026-09-20，任务 11.3 部署子项）

- 用户明确批准配置独立服务身份并短暂重启相关服务，不修改业务授权、知识资源或应用发布。起点 `5d9ce4b`、工作区干净，Docker context=`desktop-linux`、项目=`enterprise_agent`。正式库 head=140，更新前非终态 Job=0、待处理 Delivery=0，无在途文件处理；不需要新迁移，也没有重启旧 Migrator 或执行 bootstrap 种子。
- 正式构建并定向部署 API、ONES MCP、Agent Worker、Python Runtime、Knowledge MCP，统一 source_revision=`5d9ce4b`、build_id=`knowledge-online-20260920`，镜像实际 psycopg=3.3.6。ONES 的主 Compose 未传 release args，验收发现初始标识为 local-dev，随后显式传入两个 build args 重建并定向替换，代码不变。其余服务未扩大更新；切换期间临时停止 API/钉钉入口，恢复时按精确容器名启动钉钉入口，不启动其依赖。
- 显式调用固定数据库授权实现创建 `knowledge_mcp_reader`，无超级用户、继承、建库/建角色、复制及 RLS 绕过能力，无角色成员关系；精确列读取与三张审计表必要写入的完整检查通过。未更改 PUBLIC 权限、平台账号或用户配置的内容账号。维护事务禁止语句及错误语句记录，生成材料/既有凭据不进入工具输出或日志。
- 新生成独立高熵 bootstrap 和专用连接配置，保存在宿主受管目录、owner-only，未复制根 `.env`、改旧凭据或提交材料；在线 Compose 显式叠加根 `.env` 与受管 `knowledge-online.env`。Knowledge MCP 只挂只读 bootstrap 和公开 JWKS，以 10008 运行、只读根文件系统、cap_drop=ALL、无宿主端口；未获得平台主密钥、签名私钥或 ONES 凭据。只新增其内容连接出网网络，未重建数据库/Embedding/Qdrant 或数据卷。
- 正式服务健康检查通过，Web 代理 `/api/ready`=200；Knowledge MCP 使用专用账号的 schema/完整权限检查通过，运行代码确认最长 120 秒。从正式 MCP 执行内部服务身份兑换并用公开 JWKS 验证固定签名、audience、scope 及 TTL；原 Token 仅在进程内存中使用。匿名 `/mcp` 返回 401；两个平台桥在只有服务身份、没有 Knowledge Business Principal 时均返回 401/no-store，不能凭服务身份取得用户数据或连接凭据。
- 从 MCP 网络检查 Embedding ready、Qdrant health 与版本 v1.19.1，READY 索引仍有 8309 个点；没有编码查询或执行缺陷检索。部署前后资源/修订状态与发布指针、KB 授权、Agent/Application Publication、文档修订内容 hash、向量成员状态的数据库内聚合摘要全部一致，5000 条文档及 8309 分块/点保留。保留旧四个服务镜像的本机回退标签，不删除旧镜像或数据。
- 当前仍 **49/54**。此次只完成 11.3 部署子项，不保存草稿、发布知识资源、修改业务 RBAC/Publication、创建或冒用真实 Job，不调用真实 ONES 或聊天模型；11.2–11.6 不补勾。真实应用新消息、两用户权限/撤权、人工约 50 条标签及质量/时延目标仍需后续授权和验收；未提交、推送、同步主规格或归档。
- 部署后复测在线 Compose 与固定 MCP 合同 **33 passed（4.51 秒）**；正式三文件 Compose quiet、严格 OpenSpec、Markdown 链接及 diff 检查通过。ONES 补齐构建标识后重跑服务/身份/失败关闭检查仍通过，五个更新服务均运行，带 healthcheck 的服务及恢复的钉钉入口 healthy、restart_count=0。仅追加部署记录和更新手册，没有新增产品代码改动；不重复累计前轮 851 项本机回归或隔离容器子回归。
