...
test("HTTP runtime exposes passive health/version/readiness and strict NDJSON terminal", async () => {
  ...
  try {
    ...
    const response = await fetch(`${base}/internal/v1/executions`, {
      method: "POST",
      headers: {
    ...
    const cancelResponse = await fetch(
      `${base}/internal/v1/executions/${value.invocation_id}/cancel`,
      {
        method: "POST",
        headers: {
          authorization: `Bearer ${runtimeGrant}`,
    ...
    assert.equal((await cancelResponse.json()).status, "already_terminal");
  } finally {
    server.close();
