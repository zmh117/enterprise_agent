...
test("PostgreSQL role grants keep platform reads column-bounded and ledger writes isolated", async () => {
  const sql = await readFile(new URL("../../sql/001_runtime_reader_role.sql", import.meta.url), "utf8");

  assert.match(sql, /CREATE ROLE agent_runtime_reader NOLOGIN/);
  assert.match(sql, /GRANT SELECT \(id, protocol, status\) ON model_connection/);
  assert.match(
    sql,
