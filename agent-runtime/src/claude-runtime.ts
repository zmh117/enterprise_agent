...
interface ToolCallState {
  readonly serverCode: "ones-mcp" | "data-mcp";
...
export class ClaudeAgentRuntimeExecutor {
  ...
  private async run(
    ...
    const toolIndex = new Map<string, { serverCode: "ones-mcp" | "data-mcp"; toolName: string }>();
    const mcpServers: NonNullable<Options["mcpServers"]> = {};
    for (const server of request.mcp_servers) {
      const alias = server.server_code === "ones-mcp" ? "ones" : "data";
      mcpServers[alias] = {
        type: "http",
        url: server.url,
        headers: {
