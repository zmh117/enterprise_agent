...
import { Badge, Button, Card, ErrorNotice, Field, Input, PageHeader, formatTime } from "../components/ui";
import { api, jsonBody } from "../lib/api";
import type { AgentConfig, AgentPayload, AgentRevision, Connector, Publication } from "../lib/types";

const AGENT = "default-diagnostic-agent";

export function AgentPage() {
  const client = useQueryClient();
  const query = useQuery({ queryKey: ["agent", AGENT], queryFn: () => api<{ agent: AgentPayload }>(`/api/admin/agents/${AGENT}`) });
  const publications = useQuery({ queryKey: ["agent-publications", AGENT], queryFn: () => api<{ publications: Publication[] }>(`/api/admin/agents/${AGENT}/publications`) });
...
function AgentEditor({ agent, initialDraft, publications, refresh }: { agent: AgentPayload; initialDraft: AgentConfig; publications: Publication[]; refresh: () => Promise<void> }) {
  ...
  const save = useMutation({ mutationFn: () => api<{ revision: AgentRevision }>(`/api/admin/agents/${AGENT}/draft`, { method: "PUT", ...jsonBody({ expected_revision: agent.draft?.revision ?? 0, config: draft }) }), onSuccess: refresh });
  const validate = useMutation({ mutationFn: (revisionId: string) => api<{ revision: AgentRevision }>(`/api/admin/agents/${AGENT}/validate`, { method: "POST", ...jsonBody({ revision_id: revisionId }) }), onSuccess: refresh });
  const publish = useMutation({ mutationFn: (revisionId: string) => api<{ publication: Publication }>(`/api/admin/agents/${AGENT}/publish`, { method: "POST", ...jsonBody({ revision_id: revisionId }) }), onSuccess: refresh });
  const rollback = useMutation({ mutationFn: (publicationId: string) => api(`/api/admin/agents/${AGENT}/rollback`, { method: "POST", ...jsonBody({ publication_id: publicationId }) }), onSuccess: refresh });
...
function ConnectorCard({ connectors, draft, onChange }: { connectors: Connector[]; draft: AgentConfig; onChange: (next: AgentConfig) => void }) {
