...
import type { Identity, Role, Session, User } from "../lib/types";

type UserDetail = { user: User; roles: Role[]; identities: Identity[]; sessions: Session[] };
type Tenant = { connector_id: string; name: string; tenant_code: string };

export function UsersPage() {
  const queryClient = useQueryClient();
  const users = useQuery({ queryKey: ["users"], queryFn: () => api<{ users: User[] }>("/api/admin/users") });
  const roles = useQuery({ queryKey: ["roles"], queryFn: () => api<{ roles: Role[] }>("/api/admin/roles") });
  const tenants = useQuery({ queryKey: ["dingtalk-tenants"], queryFn: () => api<{ tenants: Tenant[] }>("/api/admin/dingtalk-tenants") });
  const [selectedId, setSelectedId] = useState("");
  const effectiveSelectedId = selectedId || users.data?.users[0]?.id || "";
  const detail = useQuery({
    queryKey: ["user", effectiveSelectedId],
    queryFn: () => api<UserDetail>(`/api/admin/users/${effectiveSelectedId}`),
...
function CreateUser({ onCreated }: { onCreated: (id: string) => void }) {
  ...
  const mutation = useMutation({
    mutationFn: () => api<{ user: User }>("/api/admin/users", { method: "POST", ...jsonBody({ ...form, password: form.password || null }) }),
...
function UserDetailPanel({ detail, allRoles, tenants, refresh }: { detail: UserDetail; allRoles: Role[]; tenants: Tenant[]; refresh: () => Promise<void> }) {
  const { user } = detail;
  ...
  const update = useMutation({ mutationFn: (status: User["status"]) => api(`/api/admin/users/${user.id}`, { method: "PUT", ...jsonBody({ expected_revision: user.revision, display_name: user.display_name, email: user.email, status }) }), onSuccess: refresh });
  const membership = useMutation({ mutationFn: ({ role, enabled }: { role: Role; enabled: boolean }) => {
    ...
    return api(`/api/admin/users/${user.id}/roles`, { method: "POST", ...jsonBody({ role_id: role.id, enabled, expected_revision: assigned?.membership_revision ?? 0 }) });
  }, onSuccess: refresh });
  const bind = useMutation({ mutationFn: () => api(`/api/admin/users/${user.id}/dingtalk-identities`, { method: "POST", ...jsonBody({ expected_user_revision: user.revision, tenant_code: selectedTenant, connector_id: selectedConnector, external_subject_id: binding.subject, display_name: binding.display }) }), onSuccess: async () => { setBinding({ tenant: selectedTenant, connector: selectedConnector, subject: "", display: "" }); await refresh(); } });
  const identityStatus = useMutation({ mutationFn: (identity: Identity) => api(`/api/admin/identities/${identity.id}/status`, { method: "PUT", ...jsonBody({ expected_revision: identity.revision, status: identity.status === "enabled" ? "disabled" : "enabled" }) }), onSuccess: refresh });
  const revoke = useMutation({ mutationFn: (session: Session) => api(`/api/admin/users/${user.id}/sessions/${session.id}`, { method: "DELETE" }), onSuccess: refresh });
