...
import { Badge, Button, Card, EmptyState, ErrorNotice, Field, Input, PageHeader } from "../../../shared/presentation/ui";
import { authorizationService } from "../application/services";
import type { RoleDetail } from "../domain/models";
...
export function RolesPage() {
  ...
  const detail = useQuery({ queryKey: ["role", effectiveSelectedId], queryFn: () => authorizationService.getRole(effectiveSelectedId), enabled: Boolean(effectiveSelectedId) });
...
function CreateRole({ onCreated }: { onCreated: (id: string) => void }) {
  ...
  const mutation = useMutation({ mutationFn: () => authorizationService.createRole(form), onSuccess: ({ role }) => { setOpen(false); onCreated(role.id); } });
...
function RoleDetailPanel({ detail, refresh }: { detail: RoleDetail; refresh: () => Promise<void> }) {
  ...
  const status = useMutation({ mutationFn: () => authorizationService.toggleRoleStatus(detail.role), onSuccess: refresh });
  const policy = useMutation({ mutationFn: () => authorizationService.createPermission(detail.role, form), onSuccess: refresh });
  return <>
