...
import type { WebhookCatalog, WebhookTriggerConfig, WebhookTriggerDefinition, WebhookTriggerPayload } from "../domain/models";
...
export function WebhooksPage() {
  const { user } = useAuth();
  const query = useQuery({ queryKey: ["webhook-triggers"], queryFn: webhookService.list });
...
export function WebhookEditorPage() {
  ...
  const catalogQuery = useQuery({ queryKey: ["webhook-catalog"], queryFn: webhookService.catalog });
  const triggerQuery = useQuery({
    queryKey: ["webhook-trigger", code],
    queryFn: () => webhookService.get(code ?? ""),
...
function WebhookEditor({ code, catalog, trigger, initial }: { code?: string; catalog: WebhookCatalog; trigger?: WebhookTriggerPayload; initial: WebhookTriggerConfig }) {
  const { user } = useAuth();
  ...
  const create = useMutation({
    mutationFn: () => webhookService.create({ code: newCode, name, trigger_type: config.adapter === "grafana_alertmanager_v1" ? "grafana" : "generic", connector_id: ingressConnectors[0]?.id ?? "", config }),
    onSuccess: (result) => navigate(`/admin/webhooks/${result.trigger.definition.code}`),
  });
  const save = useMutation({
    mutationFn: () => webhookService.saveRevision(code ?? "", trigger?.draft?.revision ?? 0, config),
    onSuccess: refresh,
  });
  const validate = useMutation({
    mutationFn: (revisionId: string) => webhookService.validateRevision(code ?? "", revisionId),
    onSuccess: refresh,
  });
  const previewMutation = useMutation({
    mutationFn: ({ revisionId, payload }: { revisionId: string; payload: Record<string, unknown> }) => webhookService.previewRevision(code ?? "", revisionId, payload),
    onSuccess: (result) => setPreview(result.preview),
  });
  const publish = useMutation({
    mutationFn: (revisionId: string) => webhookService.publishRevision(code ?? "", revisionId),
    onSuccess: refresh,
  });
  const rotate = useMutation({
    mutationFn: () => webhookService.rotatePublicId(code ?? "", trigger?.definition.revision),
    onSuccess: refresh,
  });
  const rollback = useMutation({
    mutationFn: (publicationId: string) => webhookService.rollback(code ?? "", publicationId, trigger?.definition.revision),
    onSuccess: refresh,
  });
  const setEnabled = useMutation({
    mutationFn: (enabled: boolean) => webhookService.setStatus(code ?? "", trigger!.definition, name, enabled),
    onSuccess: refresh,
  });
  const setServiceAccount = useMutation({
    mutationFn: (enabled: boolean) => webhookService.setServiceAccountStatus(code ?? "", trigger?.definition.revision, enabled),
    onSuccess: refresh,
  });
...
export function WebhookEventsPage() {
  ...
  const query = useQuery({ queryKey: ["webhook-events", code, status], queryFn: () => webhookService.listEvents(code, status) });
  const [selected, setSelected] = useState<string | null>(null);
  const detail = useQuery({ queryKey: ["webhook-event", selected], queryFn: () => webhookService.getEvent(selected ?? ""), enabled: Boolean(selected) });
...
function defaultConfig(catalog: WebhookCatalog): WebhookTriggerConfig {
