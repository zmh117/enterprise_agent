...
import { UsersPage } from "./contexts/identity/presentation/UsersPage";
...
describe("admin routing", () => {
  beforeEach(() => {
    ...
    vi.stubGlobal("fetch", vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (path === "/api/auth/me") return authenticated ? jsonResponse({ user: currentUser }) : jsonResponse({ detail: "not authenticated" }, 401);
      if (path === "/api/auth/login" && init?.method === "POST") {
        authenticated = true;
        return jsonResponse({ user: currentUser });
      }
      if (path === "/api/admin/users") return jsonResponse({ users: [{ id: "user-1", username: "admin", display_name: "管理员", email: "admin@example.test", status: "enabled", revision: 3 }] });
      if (path === "/api/admin/users/user-1") return jsonResponse({
        user: { id: "user-1", username: "admin", display_name: "管理员", email: "admin@example.test", status: "enabled", revision: 3 },
        roles: [{ id: "role-admin", code: "platform-admin", name: "平台管理员", description: "", status: "enabled", revision: 2, membership_revision: 1 }],
        identities: [{ id: "identity-1", user_id: "user-1", provider: "dingtalk", tenant_code: "default", external_subject_id: "03695725024624053732", connector_id: "dingtalk-default", display_name: "zmh", status: "enabled", revision: 1, last_seen_at: "2026-07-19T14:43:40Z" }],
        sessions: [{ id: "session-1", status: "active", created_at: "2026-07-20T01:00:00Z", last_seen_at: "2026-07-20T01:30:00Z", idle_expires_at: "2026-07-20T02:00:00Z", absolute_expires_at: "2026-07-21T01:00:00Z", user_agent_summary: "Chrome", remote_address_summary: "local" }],
      });
      if (path === "/api/admin/roles") return jsonResponse({ roles: [{ id: "role-admin", code: "platform-admin", name: "平台管理员", description: "", status: "enabled", revision: 2 }] });
      if (path === "/api/admin/roles/role-admin") return jsonResponse({
        role: { id: "role-admin", code: "platform-admin", name: "平台管理员", description: "", status: "enabled", revision: 2 },
        members: [{ id: "user-1", username: "admin", display_name: "管理员", email: "admin@example.test", status: "enabled", revision: 3, membership_status: "enabled" }],
        permissions: [{ id: "permission-1", subject_type: "role", subject_code: "platform-admin", resource_type: "agent", resource_code: "default-diagnostic-agent", action: "use", effect: "allow", priority: 100, status: "enabled", revision: 1 }],
      });
      if (path === "/api/admin/dingtalk-tenants") return jsonResponse({ tenants: [{ connector_id: "dingtalk-default", name: "默认钉钉企业", tenant_code: "default" }] });
      if (path === "/api/admin/audit-events?limit=300") return jsonResponse({ events: [{ id: "audit-1", event_type: "identity.bound", actor_id: "user-1", status: "SUCCEEDED", summary: "绑定钉钉身份", created_at: "2026-07-20T01:00:00Z", payload_summary: { payload: "[redacted]", truncated: false } }] });
  ...
  it("preserves login and returns to the protected destination", async () => {
    authenticated = false;
    renderApp("/admin/users");
    await waitFor(() => expect(screen.getByRole("heading", { name: "登录管理台" })).toBeInTheDocument());
    ...
    await waitFor(() => expect(screen.getByRole("heading", { name: "用户与外部身份" })).toBeInTheDocument());
  });

  it("preserves unified users, DingTalk identities, role memberships and sessions", async () => {
    const client = new QueryClient({ defaultOptions: { queries: { retry: false, staleTime: Number.POSITIVE_INFINITY } } });
    client.setQueryData(["users"], { users: [{ id: "user-1", username: "admin", display_name: "管理员", email: "admin@example.test", status: "enabled", revision: 3 }] });
    client.setQueryData(["roles"], { roles: [{ id: "role-admin", code: "platform-admin", name: "平台管理员", description: "", status: "enabled", revision: 2 }] });
    client.setQueryData(["dingtalk-tenants"], { tenants: [{ connector_id: "dingtalk-default", name: "默认钉钉企业", tenant_code: "default" }] });
    ...
    await waitFor(() => expect(screen.getByText(/03695725024624053732/)).toBeInTheDocument());
    expect(screen.getAllByText("管理员").length).toBeGreaterThan(0);
    expect(screen.getByText("登录会话")).toBeInTheDocument();
    expect(screen.getByText("Chrome")).toBeInTheDocument();
  });

  it("preserves role policies and memberships", async () => {
    renderApp("/admin/roles");
    await waitFor(() => expect(screen.getByText("default-diagnostic-agent")).toBeInTheDocument());
    expect(screen.getAllByText("allow").length).toBeGreaterThan(0);
    expect(screen.getByText("成员")).toBeInTheDocument();
  });

  it("renders only the safe audit summary", async () => {
    renderApp("/admin/audit");
    await waitFor(() => expect(screen.getByText("identity.bound")).toBeInTheDocument());
    expect(screen.getByText("绑定钉钉身份")).toBeInTheDocument();
