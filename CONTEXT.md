...
- 旧企业占位标识只允许从已保存的受信钉钉事件幂等迁移为真实 Corp ID，不得继续作为新身份命名空间
- 旧钉钉身份迁移必须保留身份 ID、人员关联、状态和使用事实，并从受信历史补齐 Corp ID、最新昵称及应用观察记录，不得要求用户重新绑定
- 迁移产生但尚未由管理员确认名称的**钉钉企业**必须明确标记“未命名／待确认”，不得以渠道名称冒充企业名称，也不得因此中断已有消息处理
...
- 本人模式展示当前用户自己的钉钉身份状态、租户和最近使用信息，但钉钉身份只读，不允许本人绑定、启停或解绑
- 钉钉身份成功解析受信消息时，以其中最新非空 `senderNick` 刷新**钉钉昵称**；空值不得清除已有昵称，管理员不得手工改写该外部事实
