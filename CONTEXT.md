...
**内置工具 Manifest（Built-in Tool Manifest）**:
由代码拥有并纳入 Implementation Digest 的工具定义，包含稳定标识、版本、名称、模型描述、输入输出 Schema、风险、只读语义、权限、资源槽和固定验证计划。
_Avoid_: Web 工具表单、数据库覆盖值、运行时动态 Schema
...
**旧版名称绑定（Legacy Name-only Binding）**:
旧 Agent Publication 只保存工具名称并由运行时标记为 `legacy-v1` 的兼容引用，不包含精确 Handler Version、Tool Release、Implementation Digest 或验证证据。
_Avoid_: Tool Release 1.0、正式 Handler 版本、长期兼容契约
...
**内置工具运行健康（Built-in Tool Operational Health）**:
根据近期调用和依赖可用性观测得到的 `HEALTHY`、`DEGRADED` 或 `UNAVAILABLE` 状态，用于排障和影响判断但不自动改写工具发布生命周期。
_Avoid_: 内置工具发布状态、工具安装状态、自动停用开关

**工具资源（Tool Resource）**:
供内置只读工具访问数据库、Redis 或 Loki 等目标的受治理资源身份，其连接内容通过验证后形成不可变发布版本。
_Avoid_: 内置只读工具、Agent 配置、明文连接凭据

**业务目标范围（Business Target Scope）**:
Job 访问业务数据时冻结的可变深度逻辑路径，可以是环境、环境／基地或环境／基地／车间；不存在的层级保持为空，不创建虚拟基地或虚拟车间。
_Avoid_: 固定三级地址、资源连接、把云端或边端编码进基地名称

**资源部署侧（Resource Placement）**:
与环境、基地、车间业务层级正交的可选资源位置限定；只有同一逻辑范围存在物理独立的云端和边端上游时才使用 `cloud` 与 `edge`，没有云边区分时该限定不存在，并由 Job 执行范围按实际拓扑冻结允许集合。
_Avoid_: 必填拓扑层级、`none` 等虚拟部署侧、业务基地、业务车间、角色或用户数据范围维度、未受 Job 允许集合约束的模型自由切换

**车间资源分区策略版本（Workshop Resource Partition Policy Revision）**:
逻辑车间在继承环境级或基地级共享资源后必须应用的不可变隔离约束版本，按实际共享资源类型包含数据库表前缀、Redis Key 命名空间前缀列表或未来可验证的 Loki 车间 Label；同一逻辑车间的云端和边端始终复用同一精确版本。
_Avoid_: 可变车间字段、按部署侧复制或覆盖、车间独立连接、重复工具资源、仅供提示而不强制执行的元数据

**Loki 范围选择器策略版本（Loki Scope Selector Policy Revision）**:
管理员根据 Loki 连接测试发现的 Label Key 与 Value，通过受控下拉为一个客户／环境及可选基地配置、验证并发布的一组不可变精确匹配条件；运行时强制注入全部条件，模型只能在该边界内增加受允许的诊断过滤，不能删除或覆盖范围选择器。
_Avoid_: 用户自填 LogQL、自由文本或正则 Selector、把物理 Label 名直接当领域层级、使用 `replica` 推断车间权限

**应用工具资源绑定（Application Tool Resource Binding）**:
Application Publication 将一个内置工具发布声明的逻辑资源槽按业务目标范围锚点和实际拓扑中可选的资源部署侧绑定到 `1..N` 个精确工具资源发布版本及有界执行约束的不可变映射。
_Avoid_: Agent 资源绑定、工具全局默认实例、浮动最新资源、顺序优先或模糊匹配
...
- **内置工具 Manifest**是模型可见行为和安全元数据的唯一事实来源；Web 只能只读展示稳定标识、名称、模型描述、Schema、风险、只读语义、权限、资源槽和验证计划，不能提供数据库覆盖值
- Web 只允许维护不进入模型上下文的 Release Note、废弃原因、替代 Tool Release 和生命周期操作原因；修改 Manifest 字段必须发布新的代码版本并重新生成验证证据
...
- `DEPRECATED` 或 `DISABLED` **内置工具发布**可以由具备 `builtin_tools.lifecycle` 的管理员恢复为 `ACTIVE`，但恢复前必须重新确认安装状态、Implementation Digest 和最新验证证据仍有效，并展示全部 Agent 与应用依赖
- 恢复操作必须填写原因并审计；恢复只改变运维状态，不修改工具内容或发布快照，既有发布引用在恢复后的下一次调用重新具备可用资格
- `ARCHIVED` 是终态且不得恢复；需要重新提供该能力时必须发布新的精确工具版本
...
- 第一版不建立内置工具发布申请、待审批状态、双人审批或按风险分级的审批工作流；机器验证通过后，单个具备 `builtin_tools.publish` 的管理员即可发布并留下完整审计
- 风险等级继续用于目录展示、验证规则和运维判断，但不改变发布审批人数；具备 `builtin_tools.lifecycle` 的单个管理员可以废弃、停用或归档，状态变更必须记录原因、操作者和时间
- **内置工具运行健康**与 `ACTIVE`、`DEPRECATED`、`DISABLED`、`ARCHIVED` 发布状态必须分开展示；瞬时超时、资源故障或错误率升高不得自动改写发布状态
- `MISSING`、`DRIFTED`、Digest 不一致或验证证据失效必须立即阻止新执行，但不得偷偷修改不可变发布内容或伪造管理员生命周期操作
- 单个应用的工具资源故障只影响相应应用资源绑定和调用，不得自动全局停用被其他应用共享的内置工具发布
...
- **业务目标范围**采用可变深度路径：环境可以直接成为叶节点，基地必须属于环境，车间必须属于基地；没有基地或车间的环境不得用占位节点伪造成固定三级拓扑
- Application Draft 必须显式选择有限的真实叶子业务目标并由 Application Publication 冻结精确 topology ID 与目标 Hash；资源映射、角色 Grant、当前 topology 全量和后续新增节点都不得隐式扩大该目标清单
- **资源部署侧**是独立于业务层级的可选限定；同一个逻辑基地和同一组逻辑车间可以同时拥有 `cloud` 与 `edge` 资源，不得通过复制为 `guanlan_cloud`、`guanlan_edge` 等伪基地表达
- 没有云边区分的有效业务范围只能绑定唯一的无部署侧资源，管理界面不得显示或要求云边字段，Job 也不得伪造 `none`、`default` 或默认 `cloud`；提交任意部署侧必须拒绝而非静默忽略
- 同一有效业务范围、内置工具发布和资源槽不得混合无部署侧绑定与 `cloud`／`edge` 绑定；存在云边区分时不得再配置无部署侧默认资源，避免回退产生跨侧访问
- `cloud`／`edge` 只表示资源路由和证据来源，不是独立授权维度；用户与角色的数据范围仍止于环境、基地和车间，同一业务目标范围获准后不再按部署侧拆分权限
- Application Publication 冻结每个业务目标范围和资源槽可用部署侧的最大集合；Job 默认继承当前业务目标范围内配置的全部部署侧，不能扩大应用发布上限，也不能用部署侧选择绕过环境、基地或车间授权
- 无云边区分的 Job 不保存部署侧字段；只授权一个部署侧时运行时直接注入该侧，同时授权 `cloud` 与 `edge` 时每次 Tool Call 必须显式选择其中一侧，缺失、未知或越界值必须失败关闭
- 同一个 Job 可以在授权集合内分别调用云端和边端资源进行同步或一致性诊断；每次 Tool Call 及其结果必须记录实际部署侧，Agent 的证据和审计不得把两侧结果合并为来源不明的数据
- 环境级或基地级数据库、Redis 工具资源可以由其后代业务目标范围共享；车间 Job 使用这些共享资源时必须额外应用该逻辑车间冻结的**车间资源分区策略版本**，不得为每个表前缀重复创建相同连接资源；第一阶段 Loki 改用独立的客户／基地范围策略，不适用本条车间继承规则
- **车间资源分区策略版本**绑定逻辑车间而不是资源部署侧；同一车间的 `cloud` 与 `edge` Tool Call 必须使用完全相同的数据库表前缀、Redis Key 命名空间前缀列表和 Loki Label，不得配置单侧覆盖值
- 第一阶段每个车间的数据库分区只允许一个非空精确表名前缀，例如 `GL001_`；不支持前缀列表、任意正则、通配模式或同一车间多前缀组合，需要扩大时必须另行变更规格
- 车间 Job 使用基地级共享数据库时，每个物理表引用都必须按数据库方言规则匹配该唯一前缀；不匹配、缺失前缀或试图引用其他车间表时必须在执行前拒绝
- 基地级 Redis 可以由该基地的多个车间共享；每个车间的分区策略必须保存一个或多个精确、可枚举且带分隔边界的 Redis Key 命名空间前缀，例如 `cr999.crmes.CRMES_TEST_GL#GL001@$`，不得仅保存 `GL001` 或使用包含判断
- Redis Tool Resource 的连接测试只验证受治理连接、认证、TLS、目标 DB 和 `PING` 等固定能力；不得在连接测试中枚举或返回业务 Key
- Redis Key 命名空间不提供全量 Key 发现或下拉选择；管理员在工具资源中心手工填写完整精确前缀，系统只能据此生成受限的 `prefix*` 模式执行有界 `SCAN` 验证，不接受管理员提交其他扫描模式
- Redis 命名空间验证只保存精确 Resource Revision、分区策略 Draft Revision、前缀内容 Hash、匹配数量、截断状态、窗口或游标边界和验证时间；响应不得展示完整业务 Key，只能返回必要的脱敏摘要
- Redis 命名空间验证零匹配仍可保存、验证和发布，但必须记录非阻断警告；系统不得自动缩短前缀、改用包含匹配或扩大 `SCAN` 范围
- 业务目标范围以环境为叶节点且不存在基地、车间时，可以只配置一个环境级 Redis；该资源不需要车间命名空间策略，Job 只能按真实的环境叶范围解析，不得创建虚拟基地、车间或伪造空分区规则
- 业务目标范围以基地为叶节点且不存在车间时，基地级 Redis 同样不需要车间命名空间策略；只有共享上级 Redis 且目标范围继续细分到车间时才要求车间 Redis Key 命名空间前缀
- 受管 Redis Key 必须遵循“固定命名空间 `#车间编码@$业务Key`”结构；命名空间根值必须精确配置而不是从环境、基地或车间名称猜测，前缀中的车间编码和分隔边界必须由机器验证
- Redis `GET` 的完整 Key 必须以当前分区策略允许的某个精确前缀开头；`SCAN` Pattern 也必须从允许前缀开始，`*GL001*` 等前置通配、任意正则或跨命名空间模式必须拒绝
- 同一车间需要增加或移除 Redis Key 命名空间时必须创建新的分区策略版本；Application Publication、Job 和重试继续冻结原精确前缀列表，不得运行时解析最新列表
- 数据库表前缀、Redis Key 命名空间前缀列表或 Loki Label 的任何变更都必须创建新的不可变**车间资源分区策略版本**并重新验证、发布，不得原地更新既有版本
- Application Publication 必须为其支持的每个车间冻结精确分区策略版本 ID、Revision 和内容 Hash；Job 执行范围继续冻结同一组事实，重试不得重新解析最新版本
- 新分区策略版本不自动改变既有 Application Publication 或 Job；需要使用新策略时必须创建并切换新的应用发布，旧策略对应的数据不可用时旧调用失败关闭，不得偷偷改用新前缀或 Label
- 第一版 Loki 必须同时支持一个平台级共享实例采集全部环境日志，以及逐步迁移到一个环境一个 Loki 实例；平台级资源作用域第一版只开放给 Loki，不据此放宽数据库或 Redis 作用域
- Loki Tool Resource Draft 保存受治理的 Base URL、Tenant、Secret 引用、超时和查询限额；Application、Agent 和 Loki 范围策略不得重复保存 URL、认证 Header、Token 或其他连接内容
- Loki 连接测试必须先验证固定受限 Endpoint、Tenant 和认证，再以有界时间窗口、数量上限和响应大小读取可选 Label Key；选择一个 Key 后，其 Value 下拉应在已选择的前置精确条件内有界发现，并展示截断状态、观察时间和刷新入口
- Label 发现结果只是连接 Draft 上的临时目录和验证证据，不直接成为运行配置；管理员必须把选择结果保存为命名 Loki 范围策略 Draft，并经过独立范围验证后发布不可变版本
- Loki 连接 Draft、连接测试、Label 发现、范围策略 Draft、范围验证和策略发布统一属于“工具资源 → Loki 资源详情”治理入口；范围策略可以被多个业务应用发布复用
- 业务应用只能从工具资源中心选择已发布且与目标 Loki Resource Revision 兼容的精确范围策略版本，不得在应用表单中重新填写 URL、Tenant、凭据、Label Key／Value 或其他 Selector 条件
- 一个**Loki 范围选择器策略版本**至少包含一个精确 `key=value` 条件；多个不同 Key 只允许按 `AND` 组合，同一 Key 不得重复，不支持 `OR`、`!=`、`=~`、任意正则、自由文本 LogQL 或脚本
- 一个策略只表达一个客户／环境及可选基地范围；需要观澜、郴州等多个范围时必须分别建立命名策略版本，不得用宽泛正则合并
- 平台级共享 Loki 必须通过精确**Loki 范围选择器策略版本**隔离业务范围；当前实例可以由管理员将环境配置为 `customer=<环境值>`，存在业务基地时再增加 `workshop=<基地值>`，但物理 Label Key 与 Value 来自受控发现和显式映射而不是代码写死
- 每个物理 Loki `customer` Label 值必须一对一映射一个独立的平台环境编码，例如 `sanjiu-test1`、`sanjiu-test2` 和 `mkk` 分别形成不同环境；不得把多个 Label 值合并为一个可执行环境范围或在查询时使用模糊别名
- 多个环境属于同一现实客户时，未来应通过 Environment 之上的 Customer／Organization 聚合表达；客户聚合不参与第一阶段 Loki 资源解析，也不能扩大任一环境的强制 `customer` Selector
- 物理 Loki Label 名不得反向污染领域术语：`workshop=guanlan` 表示观澜基地而不是 GL001 车间，`role` 表示日志采集侧而不是 RBAC 角色或 Loki 资源部署侧；管理端必须同时展示领域含义和物理 Label 映射
- 对没有基地、车间的 Docker 客户环境，第一阶段只用 `customer` 形成强制范围边界；物理 `workshop=mes-*` 和 `role=standalone` 只作为可选诊断过滤条件，不创建虚拟业务基地、车间或资源部署侧
- 当前 `role=cloud|edge|standalone`、`replica=001|002|003`、`app` 和 `logtype` 都只能作为受客户／基地范围边界约束的诊断过滤条件，不得作为 Loki 资源绑定、授权或 GL001、GL002、GL003 隔离事实
- 当前 Loki 强制隔离、资源绑定和授权粒度止于客户／环境及可选基地；在全部相关日志流提供稳定、统一且经过验证的业务车间 Label 前，不得宣称或开放车间级 Loki 隔离
- 环境级 Loki 只绑定所属环境，连接本身形成环境资源边界；仍应验证其 `customer` 等固定标签与环境策略一致，但不得与平台级 Loki 同时覆盖同一 Application Publication 的同一有效环境范围
- Loki 范围验证必须绑定精确 Loki Resource Revision、策略 Draft Revision、规范化条件内容 Hash、发现窗口和验证时间；连接测试成功不能替代范围验证，范围验证也不能修改连接资源
- Loki 范围验证不要求当前窗口至少存在一条匹配日志流；验证必须证明全部条件为精确匹配、Key 不重复、组合结构有效、目标 Loki 接受受限查询且响应未越过超时和大小边界
- 零匹配必须在验证证据中记录 `observed_series_count=0`、窗口和非阻断警告，但仍可发布；系统不得因零匹配删除条件、扩大时间或范围、改用其他 Label Value，也不得伪造已有日志的结论
- 已发布 Loki 范围在后续查询或健康探测中长期零匹配时，只更新独立运行健康为 `EMPTY` 或 `DEGRADED` 并提示检查采集链，不得自动停用、废弃或改写范围策略；查询继续在原强制 Selector 内执行并返回明确空结果
- Application Publication 必须同时冻结精确 Loki Resource Revision 与 Loki 范围策略版本 ID、Revision 和内容 Hash；Job 和重试继续冻结同一事实，不能重新读取下拉目录或解析最新策略
- 平台级或环境级 Loki 的连接切换、范围 Selector 变更都必须创建精确资源或策略新版本并由新 Application Publication 显式采用；既有发布与 Job 不自动切换，运行时必须把全部策略条件与 Agent 允许的附加过滤按 `AND` 强制合并，并拒绝删除、覆盖或扩大范围条件
- **应用发布**通过**应用工具资源绑定**为已选内置工具的每个必需资源槽冻结 `1..N` 个“业务目标范围锚点 + 可选资源部署侧 → 精确工具资源版本及约束”的映射；Agent Publication 不保存具体资源实例
- 每个绑定只能使用 Tool Manifest 对该资源槽允许的资源类型和范围层级；同一 Application Publication、内置工具发布、资源槽和资源部署侧下不得存在覆盖同一有效叶范围的多个绑定，环境级与基地级绑定若会同时命中同一 Job 也视为重叠并拒绝发布
- 每次 Tool Call 必须使用 Job 冻结的**业务目标范围**和本次调用在授权集合内选择的可选**资源部署侧**解析出唯一一个应用工具资源绑定，再应用车间分区策略；零个或多个匹配都必须失败关闭，不得使用默认资源、列表首项、浮动最新版本或模糊回退
...
- `legacy-v1` 只是**旧版名称绑定**的内部兼容标记，不是正式工具版本；管理端只能显示“旧版名称绑定，需要升级”，不得把它作为可选择或可发布版本
- 迁移必须为现有工具建立已验证的精确 Tool Release，为活动 Agent 和 Application 创建并切换到精确发布链，并为未结束或可重试的旧 Job 固化精确执行绑定
- 内置工具迁移必须分为两个可独立验证和回滚的上线阶段：第一阶段增加精确模型、禁止新旧式写入并切换活动发布，但保留旧式读取；第二阶段只在旧式活动依赖清零后删除 `legacy-v1` 运行分支和写入兼容逻辑
- 第一阶段必须输出活动 Agent、应用部署以及待执行或可重试 Job 的旧式依赖报告；第二阶段必须以报告为零和迁移后真实运行验收通过作为前置门禁
- 新写入接口必须拒绝名称绑定和 `legacy-v1`；旧发布在切换后不得重新激活或回滚为活动运行版本
- 只有活动 Agent、应用部署和非终态 Job 的 `legacy-v1` 依赖全部清零并完成验证后，才能删除运行时兼容分支；旧 Agent、应用、Job 和 Tool Call 继续作为只读审计历史保留
- 内置工具迁移完成的判定是生产运行和新写入均不再依赖 `legacy-v1`，不是物理删除历史记录
